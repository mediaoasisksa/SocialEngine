...
// 			}
    ...
    function onInstall() {
        ...
        $inviteTable = $db->query("SHOW TABLES LIKE 'engine4_seaocore_invites'")->fetch();
        if (empty($inviteTable)) {
            $db->query("CREATE TABLE IF NOT EXISTS `engine4_seaocore_invites` (
        ...
        $db->query("INSERT IGNORE INTO `engine4_core_tasks` (`title`, `module`, `plugin`, `timeout`) VALUES ('SEAO - Background Inviter', 'seaocore', 'Seaocore_Plugin_Task_Invite', 15);");
        $column_inviter_id_exist = $db->query("SHOW COLUMNS FROM engine4_seaocore_invites LIKE 'inviter_id'")->fetch();
        if (empty($column_inviter_id_exist)) {
            $db->query("ALTER TABLE `engine4_seaocore_invites` ADD `inviter_id` int(11) NOT NULL");
        }
        ...
        $column_occurrence_id_exist = $db->query("SHOW COLUMNS FROM engine4_seaocore_invites LIKE 'occurrence_id'")->fetch();
        if (empty($column_occurrence_id_exist)) {
            $db->query("ALTER TABLE `engine4_seaocore_invites` ADD `occurrence_id` int(11) NOT NULL");
        }
