...
<script type="text/javascript">
 ...
 var url = 'seaocore/favourite/favouritelist';// URL where send ajax request.
	function show_myfriend () {
		scriptJquery('#lists_popup_content').html('<center><img src="application/modules/Seaocore/externals/images/spinner.gif" /></center>');
      en4.core.request.send(scriptJquery.ajax({
        ...
        'data' : {
          ...
          'search' : this.value,
			...
			scriptJquery('#lists_popup_content').html(responseHTML);
			}
    }));
  }
    ...
    scriptJquery('#like_members_search_input')keyup(function(e) {
		scriptJquery('#lists_popup_content').html('<center><img src="<?php echo $this->layout()->staticBaseUrl?>application/modules/Seaocore/externals/images/spinner.gif" alt="" class="mtop_10" /></center>');
      en4.core.request.send(scriptJquery.ajax({
        'url' : url,
        'dataType':'html',
        'data' : {
	...
	var paginateLikeMembers = function(page, call_status) {
		var search_value = scriptJquery('#like_members_search_input').val();
    ...
    en4.core.request.send(scriptJquery.ajax({
      ...
      'element' : scriptJquery('#like_members_profile').parent()			
    });
  }
</script>
			...
			<?php 
				...
				elseif($this->resource_type == 'list_listing' || $this->resource_type == 'sitereview_listing') {
					...
					$module_name = 'event';
                                ...
                                elseif($this->resource_type == 'sitevideo_channel') {
					$module_name = 'channel';
				}
                                else {
				...
				if($this->call_status == 'public') {
					$title = $this->translate('People Who Favourite This %s', ucfirst($module_name));
				}else {
					$title = $this->translate('Friends Who Favourite This %s', ucfirst($module_name));
				} 
			?>
		...
		foreach( $this->user_obj as $user_info ) {?>
			<div class="item_member">
				<div class="item_member_thumb">
					<?php echo $this->htmlLink($user_info->getHref(), $this->itemPhoto($user_info, 'thumb.icon', $user_info->getTitle()), array('class' => 'item_photo', 'target' => '_parent', 'title' => $user_info->getTitle(), 'target' => '_parent'));?>
				</div>
