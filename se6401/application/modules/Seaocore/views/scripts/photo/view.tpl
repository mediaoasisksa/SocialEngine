...
<div class="photo_lightbox_cont">
  ...
  <div class="lightbox_btm_bl">
    ...
    <!--            <a href="http://pinterest.com/pin/create/button/?url=<?php echo urlencode(((!empty($_ENV["HTTPS"]) && 'on' == strtolower($_ENV["HTTPS"])) ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . $this->photo->getHref()); ?>&media=<?php echo urlencode((!preg_match("~^(?:f|ht)tps?://~i", $this->photo->getPhotoUrl()) ? (((!empty($_ENV["HTTPS"]) && 'on' == strtolower($_ENV["HTTPS"])) ? "https://" : "http://") . $_SERVER['HTTP_HOST'] ) : '') . $this->photo->getPhotoUrl()); ?>&description=<?php echo $this->photo->getTitle(); ?>" class="pin-it-button" count-layout="horizontal"  id="new_pin" >Pin It</a>-->
            <a  data-href="http://pinterest.com/pin/create/button/?url=<?php echo urlencode(((!empty($_ENV["HTTPS"]) && 'on' == strtolower($_ENV["HTTPS"])) ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . $this->photo->getHref()); ?>&media=<?php echo urlencode((!preg_match("~^(?:f|ht)tps?://~i", $this->photo->getPhotoUrl()) ? (((!empty($_ENV["HTTPS"]) && 'on' == strtolower($_ENV["HTTPS"])) ? "https://" : "http://") . $_SERVER['HTTP_HOST'] ) : '') . $this->photo->getPhotoUrl()); ?>&description=<?php echo $this->photo->getTitle(); ?>" class="pin-it-button" count-layout="horizontal"  id="new_light_pin" onclick="window.open(this.attr('data-href'), 'pin' + (new Date).getTime(), 'status=no,resizable=yes,scrollbars=yes,personalbar=no,directories=no,location=no,toolbar=no,menubar=no,width=632,height=270,left=0,top=0');"><img border="0" src="http://assets.pinterest.com/images/PinExt.png" title="Pin It" /></a>
            <script type="text/javascript" >
    ...
    new Asset.javascript('http://assets.pinterest.com/js/pinit.js', {});
        ...
        <?php if ($this->canEdit): ?>
          <div class="lightbox_btm_bl_rop" id="">
            <a class="icon_photos_lightbox_rotate_ccw" onclick="scriptJquery(this).attr('class', 'icon_loading');

        en4.photoadvlightbox.rotate(<?php echo $this->photo->getIdentity() ?>, 90, '<?php echo $this->resource_type; ?>').complete( function() {
          scriptJquery(this).attr('class', 'icon_photos_lightbox_rotate_ccw')
        }.bind(this));" title="<?php echo $this->translate("Rotate Left"); ?>" ></a>
            <a class="icon_photos_lightbox_rotate_cw" onclick="scriptJquery(this).attr('class', 'icon_loading');
        en4.photoadvlightbox.rotate(<?php echo $this->photo->getIdentity() ?>, 270, '<?php echo $this->resource_type; ?>').complete( function() {
          scriptJquery(this).attr('class', 'icon_photos_lightbox_rotate_cw')
        }.bind(this));" title="<?php echo $this->translate("Rotate Right"); ?>" ></a>
            <a class="icon_photos_lightbox_flip_horizontal" onclick="scriptJquery(this).attr('class', 'icon_loading');
        en4.photoadvlightbox.flip(<?php echo $this->photo->getIdentity() ?>, 'horizontal', '<?php echo $this->resource_type; ?>').complete( function() {
          scriptJquery(this).attr('class', 'icon_photos_lightbox_flip_horizontal')
        }.bind(this));" title="<?php echo $this->translate("Flip Horizontal"); ?>" ></a>
            <a class="icon_photos_lightbox_flip_vertical" onclick="scriptJquery(this).attr('class', 'icon_loading');
        en4.photoadvlightbox.flip(<?php echo $this->photo->getIdentity() ?>, 'vertical', '<?php echo $this->resource_type; ?>').complete( function() {
          scriptJquery(this).attr('class', 'icon_photos_lightbox_flip_vertical')
        ...
        if (scriptJquery('#comment-form-open-li_<?php echo $this->subject()->getType() ?>_<?php echo $this->subject()->getIdentity() ?>')) {
          scriptJquery('#comment-form-open-li_<?php echo $this->subject()->getType() ?>_<?php echo $this->subject()->getIdentity() ?>').css('display', 'none');
        }
        scriptJquery('#comment-form_<?php echo $this->subject()->getType() . "_" . $this->subject()->getIdentity() ?>').css( 'display', '' );
        scriptJquery('#comment-form_<?php echo $this->subject()->getType() . "_" . $this->subject()->getIdentity() ?>').find('body').focus();"  ><?php echo $this->translate('Comments'); ?></span>  
              <?php endif; ?>     
  ...
  function getTaggerInstanceSEAO() {
    if (!scriptJquery('#media_photo_next').length)
      return;
    ...
    var onclickNext = scriptJquery('#media_photo_next').attr('onclick');
    scriptJquery(taggerInstanceSEAO).on({
      'onBegin': function() {
        scriptJquery('#media_photo_next').attr('onclick', '');
      },
      'onEnd': function() {
        scriptJquery('#media_photo_next').attr('onclick', onclickNext);
      },
      ...
      'onRemoveTag': function(id) {
        ...
        if (existingTags.length < 1)
          scriptJquery("#media_tags").css('display', "none");
      }
    });
  ...
  en4.core.runonce.add(function() {
    ...
    var descEls = scriptJquery('.lightbox_photo_description');
    if (descEls.length > 0) {
      descEls[0].enableLinks();
    }

    if (scriptJquery('#editor_seaocore_description').length)
      scriptJquery('#editor_seaocore_description').autogrow();
     scriptJquery('#ads').css('bottom', '-500px');
    resetPhotoContentSEAO();
    (function() {
      if (!scriptJquery('#main_right_content_area').length)
        return;
      rightSidePhotoContent = SEAOMooVerticalScroll('main_right_content_area', 'main_right_content', {});
    }).delay(500);
  });
  ...
  var keyDownEventsSEAOCorePhoto = function(e) {
    if (e.target.tagName == 'HTML' ||
            ...
            (e.target.tagName == 'DIV' && !e.target.parents('.compose-container')) ||
...
<?php if ($this->canComment && $this->viewPermission): ?>
      if (e.key == 'l' && (
        ...
        if (scriptJquery('#'+photo_like_id + "unlike_link").length && scriptJquery('#'+photo_like_id + "unlike_link").css('display') == "none") {
          en4.seaocore.likes.like('<?php echo $this->subject()->getType() ?>', '<?php echo $this->subject()->getIdentity() ?>', '1');
        } else if (scriptJquery('#'+photo_like_id + "like_link").length && scriptJquery('#'+photo_like_id + "like_link").css('display') == "none") {
      ...
      en4.core.runonce.add(function() {
        ...
        var rating_over = window.rating_over = function(rating) {
          if (rated == 1 && update_permission == 0) {
            scriptJquery('#rating_text').html("<?php echo $this->translate('you already rated'); ?>");
            //set_rating();
          } else if (viewer == 0) {
            scriptJquery('#rating_text').html("<?php echo $this->translate('please login to rate'); ?>");
          } else {
            scriptJquery('#rating_text').html("<?php echo $this->translate('click to rate'); ?>");
            for (var x = 1; x <= 5; x++) {
              if (x <= rating) {
        ...
        var rate = window.rate = function(rating) {
          scriptJquery('#rating_text').html("<?php echo $this->translate('Thanks for rating!'); ?>");
...
<script type="text/javascript">
    ...
    function checkPasswordProtection(obj) {
        ...
        var flag = true;
        if (scriptJquery('#password_error').length)
            scriptJquery('#password_error').destroy();

        if (obj['password'] && obj['password'].val() == '') {
            liElement = scriptJquery.crtEle('span', {'html': '<?php echo $this->translate("* Please complete this field - it is required."); ?>', 'class': 'sitealbum_protection_error', 'id': 'password_error'}).appendTo(scriptJquery('#password-element'));
            flag = false;
        }
            ...
            var request = scriptJquery.ajax({
                ...
                success: function (responseJSON) {
                    if (responseJSON.status == 0) {
                        ...
                        liElement = scriptJquery.crtEle('span', {'html': '<?php echo $this->translate("This is not valid password. Please try again."); ?>', 'class': 'sitealbum_protection_error', 'id': 'password_error'}).appendTo(scriptJquery('#password-element'));
                        flag = false;
                    } else {
                        closeSEAOLightBoxAlbum();
