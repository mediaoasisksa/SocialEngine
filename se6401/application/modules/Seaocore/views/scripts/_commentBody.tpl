 ...
 */
?>
<?php $content = nl2br($comment->body); ?>
<?php
...
if (isset($comment->params) && !empty($comment->params)) {
    ...
    if (isset($actionParams['tags'])) {
        ...
        echo $this->smileyToEmoticons(Engine_Text_Emoji::decode($content));
    } else {
        echo $this->smileyToEmoticons($this->viewMore(Engine_Text_Emoji::decode($content),null,null,null,null));
    }
} else {
    echo $this->smileyToEmoticons($this->viewMore(Engine_Text_Emoji::decode($content),null,null,null,null));
}
