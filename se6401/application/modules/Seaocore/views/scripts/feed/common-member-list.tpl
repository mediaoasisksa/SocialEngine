...
<?php //if(!empty ($this->showViewMore)): ?>
  <script type="text/javascript">
    ...
    function hideViewMoreLink(){
        if(scriptJquery('#friend_pops_view_more').length)
            scriptJquery('#friend_pops_view_more').css('display', '<?php echo ( $this->paginator->count() ==
...
// 			}
      $item = Engine_Api::_()->getItem('user', $resouceId);
       ...
       (Engine_Api::_()->seaocore()->canSendUserMessage($item))) {
          ...
          <a onClick="window.parent.pm.openChatBox(<?php echo $resouceId ?>);" class="buttonlink primemessenger_icon" href="javascript:void(0)"><?php echo $this->translate('Message'); ?></a>
          <?php else : ?>
          <a class="buttonlink" href="<?php echo $this->base_url; ?>/messages/compose/to/<?php echo echo $resouceId ?>">
