...
<?php //if(!empty ($this->showViewMore)): ?>
  <script type="text/javascript">
    ...
    function hideViewMoreLink(){
        if(scriptJquery('#mutual_friend_pops_view_more').length)
            scriptJquery('#mutual_friend_pops_view_more').css( 'display', '<?php echo ( $this->paginator->count() == $this->paginator->getCurrentPageNumber()  )|| $this->count == 0 ? 'none' : '' ) ?>';
    }
...
<?php if (empty($this->showViewMore)): ?>
  <div class="seaocore_members_popup seaocore_members_popup_notbs">
    <div class="top">
      <div class="heading"><?php echo $this->translate('Friends')?>
...
$this->itemPhoto($this->user($user_subject), 'thumb.icon') ?></a>
    ...
    <div class="item_member_option">
       ...
       (Engine_Api::_()->seaocore()->canSendUserMessage($item))) {
          ...
          <a onClick="window.parent.pm.openChatBox(<?php echo $value['user_id'] ?>);" class="buttonlink primemessenger_icon" href="javascript:void(0)"><?php echo $this->translate('Message'); ?></a>
          <?php else : ?>
          <a class="seaocore_message" href="<?php echo $this->base_url; ?>/messages/compose/to/<?php echo $value['user_id'] ?>" >
