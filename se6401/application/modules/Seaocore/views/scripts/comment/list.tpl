  ...
  en4.core.runonce.add(function() {
    ...
    scriptJquery('.comments_comment_likes').on('mouseover', function(event) {
      ...
      if( !el.data('tip-loaded') ) {
        ...
        // Load the likes
        scriptJquery.ajax({
          url : '<?php echo $this->url(array('module' => 'core', 'controller' => 'comment', 'action' => 'get-likes'), 'default', true) ?>',
          data : { format : 'json', type : 'core_comment', id : el.attr('id').match(/\d+/)[0] },
      ...
      var tooltipHtml = '<div class="comments_comment_likes_tips">' + '<div class="tip-top"></div>' +
                  '<div class="tip">' + '<div class="tip-title">' + el.data('tip:title') + '</div>' + 
                  '<div class="tip-text">' + el.data('tip:text') + '</div>' +
                  '</div><div class="tip-bottom"></div></div>';
      scriptJquery(tooltipHtml).appendTo('body');
      scriptJquery('div.comments_comment_likes_tips').css({top: event.pageY + 8, left: event.pageX - 8});

    });

    scriptJquery('.comments_comment_likes').on('mouseleave', function(event) {
      scriptJquery('div.comments_comment_likes_tips').remove();
    });
  });
</script>
...
?></span>
    ...
    <?php if( isset($this->form) ): ?>
      - <a href='javascript:void(0);' onclick="if(scriptJquery('#comment-form-open-li_<?php echo $this->subject->getType()?>_<?php echo $this->subject->getIdentity() ?>').length){ scriptJquery('#comment-form-open-li_<?php echo $this->subject->getType()?>_<?php echo $this->subject->getIdentity() ?>').css('display', 'none'); } scriptJquery('#comment-form_<?php echo $this->subject->getType()?>_<?php
echo $this->subject->getIdentity() ?>').css('display', '');if(document.getElementById('comment-form_<?php echo
$this->subject->getType()?>_<?php echo $this->subject->getIdentity() ?>').body) {scriptJquery('#comment-form_<?php echo $this->subject->getType()?>_<?php echo $this->subject->getIdentity() ?>').find('#body').focus();}"><?php echo $this->translate('Post Comment') ?></a>
...
"'.$this->subject->getIdentity().'", "'.($this->page + 1).'","'. $this->show_bottom_post .'")'
      ...
      <li id='comment-form-open-li_<?php echo $this->subject->getType()?>_<?php echo $this->subject->getIdentity() ?>' onclick="scriptJquery(this).css('display', 'none'); scriptJquery('#comment-form_<?php echo $this->subject->getType()?>_<?php echo $this->subject->getIdentity() ?>').css('display',  '');
        if(document.getElementById(('comment-form_<?php echo $this->subject->getType()?>_<?php echo $this->subject->getIdentity() ?>').body)) { scriptJquery('#comment-form_<?php echo $this->subject->getType()?>_<?php echo $this->subject->getIdentity() ?>').find('#body').focus();}"> 

        <div/></div>
        <div class="seaocore_comment_box seaocore_txt_light dnone"><?php echo $this->translate('Post a comment...') ?></div>
      </li>
...
?></span>
  ...
  <script type="text/javascript">
    ...
    <?php if($this->submit_post):?>
      if(scriptJquery('#comment-form-open-li_<?php echo $this->subject->getType()?>_<?php echo $this->subject->getIdentity() ?>').length){ document.getElementById('comment-form-open-li_<?php echo $this->subject->getType()?>_<?php echo $this->subject->getIdentity() ?>').css( 'display', 'none' ); } document.getElementById('comment-form_<?php echo $this->subject->getType()?>_<?php echo $this->subject->getIdentity() ?>').css( 'display', '' );scriptJquery('#comment-form_<?php echo $this->subject->getType()?>_<?php echo $this->subject->getIdentity() ?>').find('#body').focus()
