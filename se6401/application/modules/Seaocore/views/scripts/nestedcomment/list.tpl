...
<script type="text/javascript">
  ...
  en4.core.runonce.add(function() {
    ...
    scriptJquery('.seaocore_replies_comment_likes').on('mouseover', function(event) {
      var el = scriptJquery(event.target);
      if( !el.data('tip-loaded', false) ) {
        el.daa('tip-loaded', true);
        el.daa('tip:title', '<?php echo $this->translate('Loading...') ?>');
        el.daa('tip:text', '');
        ...
        var req = scriptJquery.ajax({
          ...
          success : function(responseJSON) {
             el.daa('tip:title', responseJSON.body);
             el.daa('tip:text', '');
    ...
    // Add tooltips
    ReplyLikesTooltips = new Tips(scriptJquery('.seaocore_replies_comment_likes'), {
      fixed : true,
      // title:'',
     ...
     // Enable links
    scriptJquery('.seaocore_replies_body').enableLinks();
...
<div id="like_comments" style="display:none;"></div>
 ...
 <ul>
  ...
  <script type="text/javascript">
    en4.core.runonce.add(function(){
      // scriptJquery(document.getElementById('comments-form_<?php echo $this->nested_comment_id?>').body).autogrow();

 scriptJquery(document.getElementById('comments-form_<?php echo $this->nested_comment_id?>').body).each(function () {
      ...
      // en4.seaocore.nestedcomments.test();

      en4.seaocore.nestedcomments.attachCreateComment(document.getElementById('comments-form_<?php echo $this->nested_comment_id?>'),'<?php echo
      $this->subject->getType()?>',<?php echo $this->subject->getIdentity() ?> , '<?php echo $this->parent_comment_id ?>');

      // en4.seaocore.nestedcomments.attachCreateComment(scriptJquery('#comments-form_<?php echo $this->nested_comment_id?>'),'<?php echo $this->subject->getType()?>',<?php echo $this->subject->getIdentity() ?>,'<?php echo $this->parent_comment_id ?>');
...
<script type="text/javascript">
  ...
  function showData(option,id) {
    if(option == 1) {
      scriptJquery('#seaocore_data-'+id).css('display', 'none');
      if(scriptJquery('#comment-'+id).length)
      scriptJquery('#comment-'+id).addClass("seaocore_replies_list seaocore_comments_hide");      
      scriptJquery('#show_'+id).css('display', 'block');
      scriptJquery('#hide_'+id).css('display', 'none');
    } else {
      scriptJquery('#seaocore_data-'+id).css('display', 'block');
         
      scriptJquery('#show_'+id).css('display', 'none');
      scriptJquery('#hide_'+id).css('display', 'block');
      if(scriptJquery('#comment-'+id).length)
      scriptJquery('#comment-'+id).addClass("seaocore_replies_list"); 
    }
  }
  ...
  function showReplyForm(type, id, comment_id) {
    if(scriptJquery('#comments-form_'+type+'_'+id+'_'+comment_id).css('display') == 'none') {
      scriptJquery('#comments-form_'+type+'_'+id+'_'+comment_id).css('display', 'block');
      // scriptJquery('#comments-form_'+type+'_'+id+'_'+comment_id).body.focus();
    } 
    else {
      scriptJquery('#comments-form_'+type+'_'+id+'_'+comment_id).css('display', 'none');
    }
  }
</script>
