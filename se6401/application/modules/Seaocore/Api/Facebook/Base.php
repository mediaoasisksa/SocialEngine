 ...
 */
abstract class Seaocore_Api_Facebook_Base
{
  ...
  protected function getAccessTokenFromCode($code, $redirect_uri = null)
  {
    ...
    try {
      ...
      $accessTokenResponse = $this->_oauthRequest(
      ...
      $log->log($e, Zend_Log::WARN);
      return false;
    }

    if( empty($accessTokenResponse) ) {
      $log->log('Facebook Access Token error : Facebook Access Token response is empty', Zend_Log::WARN);
      return false;
    }

    $responseParams = json_decode($accessTokenResponse, true);
    if( !isset($responseParams['access_token']) ) {
      $log->log('Facebook Access Token error : Facebook Access Token is not set', Zend_Log::WARN);
      return false;
    }
  ...
  protected function _restserver($params)
  {
    ...
    $result = Zend_Json::decode($this->_oauthRequest(
  ...
  protected function _oauthRequest($url, $params)
  {
    if( !isset($params['access_token']) ) {
      ...
      if( (isset($session->aaf_fbaccess_token) && !empty($session->aaf_fbaccess_token) ) ) {
        $params['access_token'] = $session->aaf_fbaccess_token;
      } else if( (isset($_SESSION['aaf_fbaccess_token_js']) && !empty($_SESSION['aaf_fbaccess_token_js']) ) ) {
        $params['access_token'] = $_SESSION['aaf_fbaccess_token_js'];
        ...
        if( !empty($fb_uid['access_token']) ) {
          $params['access_token'] = $fb_uid['access_token'];
        } else {
  ...
  protected function parseSignedRequest($signed_request)
  {
    ...
    $data = Zend_Json::decode(self::base64UrlDecode($payload), true);
   ...
   * @return mixed
   */
  abstract public function getPersistentData($key, $default = false);
  /**
   ...
   * @return void
   */
  abstract public function clearPersistentData($key);
  /**
   * Clear all data from the persistent storage
   *
   * @return void
   */
  abstract public function clearAllPersistentData();
}
