...
class Seaocore_Api_Facebook_Facebookinvite extends Core_Api_Abstract
{
      ...
      if( (isset($session->aaf_fbaccess_token) && !empty($session->aaf_fbaccess_token) ) ) {
        ...
        if( !is_object(json_decode($result)) && is_string($result) ) {
          ...
          if( !empty($result->access_token) ) {
            if( !empty($result->access_token) ) {
              $fbapi->setAccessToken($result->access_token);
  ...
  public function getFBPermittedPages($fbapi, $ids_assoc = false)
  {
    if( !$fbapi )
      return false;
      ...
      if( !empty($pages) && isset($pages['data']) && !empty($pages['data']) ) {
        foreach( $pages['data'] as $page ) {
          ...
          if( isset($page['id']) && !empty($page['id']) ) {
            $page_details = $fbapi->api('/' . $page['id'] . '?fields=id,name,link');
            if( !empty($page_details) && isset($page_details['link']) ) {
              $pages_detail_assoc[$page_details['link']] = $page_details['name'];
              $pages_ids_assoc[$page_details['link']] = $page_details['id'];
            }
          }
        }
      }
    ...
    if( empty($ids_assoc) ) {
      ...
      return $pages_ids_assoc;
    }
  }

  public function getFBPermittedGroups($fbapi)
  {
    if( !$fbapi )
      return false;

    $groups_ids_assoc = array(0 => '----Select----');

    try {
      $groups = $fbapi->api('/me/groups');

      if( !empty($groups) && isset($groups['data']) && !empty($groups['data']) ) {
        foreach( $groups['data'] as $group ) {

          if( isset($group['id']) && !empty($group['id']) ) {
            $groups_ids_assoc[$group['id']] = $group['name'];
          }
        }
      }
    } catch( Exception $e ) {
