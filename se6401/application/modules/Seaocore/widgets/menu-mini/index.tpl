  ...
  <script type="text/javascript">
    ...
    window.addEventListener('DOMContentLoaded', function(){
      if ('<?php echo $infoCurrency; ?>' == 'countryFlag') {
        document.getElementById('flag_display').src = document.getElementById('flag_display').src + '<?php echo Engine_Api::_()->sitemulticurrency()->getSelectedCurrency(); ?>.png';
      } else {
...
<div id='core_menu_mini_menu'>
  <ul class="seaocore_mini_menu_items <?php echo empty($this->showIcons) ? ' _show_icon_labels' : ''; ?>">
    <?php foreach( $this->navigation as $item ): ?>
      ...
      <?php if( in_array($menuName, array('core_mini_admin', 'core_mini_settings', 'core_mini_friends')) ): ?>
        <?php continue; ?>           
      <?php endif; ?>
      ...
      <li class="seaocore_mimi_menu_item seaocore_mimi_menu_<?php echo $menuName; ?> <?php echo in_array($menuName, $this->pullDownMiniMenus) ? ' seaocore_mimi_menu_pulldown_item' : ' ' ?>">
        ...
        <a <?php echo $xhtml; ?>>
          ...
          <?php endif; ?>
          <?php $icon = $item->get('icon');?>  
          <span class="_icon <?php echo $icon ? (!Zend_Uri::check($icon) ? $icon  : '_bk_icon') : '_no_icon'?>" style="<?php if($icon && Zend_Uri::check($icon)):?> background-image:url(<?php echo $icon ?>)<?php endif; ?>"></span>
