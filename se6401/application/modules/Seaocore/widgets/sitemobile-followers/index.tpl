...
var followersSitemobile = function(call_status) {
	...
	if(scriptJquery.mobile.activePage.find("#global_page_sitepage-index-view")) {
		urlRequest = sm4.core.baseUrl + 'sitepage/mobile-widget/index/content_id/' + <?php echo $this->identity?>;
	} else if(scriptJquery.mobile.activePage.find("#global_page_sitebusiness-index-view")) {
		urlRequest = sm4.core.baseUrl + 'sitebusiness/mobile-widget/index/content_id/' + <?php echo $this->identity?>;
	} else if(scriptJquery.mobile.activePage.find("#global_page_sitegroup-index-view")) {
	...
	sm4.core.request.send({
		...
		url : urlRequest,
		data: {
			'subject': sm4.core.subject.guid != '' ? sm4.core.subject.guid : scriptJquery.mobile.activePage.attr("data-subject"),  
			'format':'html',
			'call_status' : call_status,
      'isajax' : 1
		}
		},{
			'element' : scriptJquery.mobile.activePage.find("#profile_pagefollwers"),
			'showLoading': true
		}
	);
};
...
<?php if(empty($this->isajax)):?>
	<div class="sm-content-list">
		<select name="auth_view" onchange="followersSitemobile(scriptJquery(this).val());" >
