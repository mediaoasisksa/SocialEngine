...
<div id="region_<?php echo $this->widgetContentId; ?>" class="sitecitycontent_dropdown">
    ...
    <script type="text/javascript">
        ...
        scriptJquery('.location_arrow_wrap_<?php echo $this->widgetContentId;?>').on('click', function() {
            scriptJquery('#locationValues_<?php echo $this->widgetContentId; ?>').toggle();
        });        
        
        scriptJquery(document).on('click', function(e) {
            
            var dropdownElement = scriptJquery(e.target);  
            if(dropdownElement.parents('.sitecitycontent_dropdown')){
               dropdownElement = dropdownElement.parents('.sitecitycontent_dropdown'); 
            }
            
            if(!dropdownElement.hasClass('sitecitycontent_dropdown') && scriptJquery('#locationValues_<?php echo $this->widgetContentId; ?>').css('display') === 'block'){
             scriptJquery('#locationValues_<?php echo $this->widgetContentId; ?>').css('display', 'none');
            }
        });
        
        function changeSpecificLocation(specificLocation) {
            scriptJquery('#current_url').val(location.pathname + location.search);
            scriptJquery('#specificLocation').val(specificLocation);
            scriptJquery('#specific_location_form').submit();   
        }
    </script>    
<?php else:  ?>
                      ...
                      <div class="form-elements">
                          ...
                          <div class="form-wrapper" id="buttons-wrapper" style="display: block;margin-bottom: 0;">
                              ...
                              <div class="form-element" id="buttons-element">
                                  <button type="submit" id="execute" name="execute" onclick="changeLocationSubmitForm(document.getElementById('seaocore_change_my_location'));
    ...
    <script type="text/javascript">
        ...
        function unsetLatLng() {
            scriptJquery('#latitude').val(0);
            scriptJquery('#longitude').val(0);
        }
        ...
        function changeMyLocation() {  
            Smoothbox.open('<div id="changeMyLocationHTML">' +  document.getElementById('changeMyLocation').innerHTML + '</div>');
            var autocomplete = new google.maps.places.Autocomplete(scriptJquery('#changeMyLocationHTML').find('#changeMyLocationValue')[0]);
            google.maps.event.addListener(autocomplete, 'place_changed', function() {
                ...
                scriptJquery('#latitude').val(place.geometry.location.lat());
                scriptJquery('#longitude').val(place.geometry.location.lng());
            });
        } 
        ...
        function changeLocationSubmitForm(formObject) { 
            ...
            var newLocationValue = scriptJquery('#changeMyLocationHTML').find('#changeMyLocationValue').val();
            var latitude = scriptJquery('#changeMyLocation').find('#latitude').val();
            var longitude = scriptJquery('#changeMyLocation').find('#longitude').val();
            var updateCookies = 1;
            if(scriptJquery('#changeMyLocationHTML').find('#removeLocation').length) {
                var removeLocationValue = scriptJquery('#changeMyLocationHTML').find('#removeLocation')[0].checked;
            ...
            if(scriptJquery(formObject).find('#location_privacy').length) {
              location_privacy = scriptJquery(formObject).find('#location_privacy')[0].options[selectedIndex].value;
            }
            
            scriptJquery('#changeMyLocationHTML').html('<center><div class="seaocore_content_loader"></div></center>');
            ...
            en4.core.request.send(scriptJquery.ajax({
                ...
                success: function(responseJSON) { 
                    if(responseJSON.error == 2) {
                        scriptJquery('#changeMyLocationHTML').find('#changeMyLocationValueErrorGeo').css('display', 'block');
                    }   
                    else if(responseJSON.error == 1) {
                        scriptJquery('#changeMyLocationHTML').find('#changeMyLocationValueError').css('display', 'block');
                    }    
                    else {
                       ...
                       myLocationDetails = scriptJquery.extend(myLocationDetails,{
                       ...
                       myLocationDetails = Object.assign(myLocationDetails,{
                            locationmiles : <?php echo Engine_Api::_()->getApi('settings', 'core')->getSetting('seaocore.locationdefaultmiles', 1000); ?>
                       });
                       }                  

                        myLocationDetails = scriptJquery.extend(myLocationDetails, {changeLocationWidget:1});
