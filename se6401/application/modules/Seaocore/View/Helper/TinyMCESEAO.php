...
class Seaocore_View_Helper_TinyMCESEAO extends Zend_View_Helper_Abstract {
  ...
  public function addJS($onlyPath = false) {
    ...
    if (empty(Engine_Api::_()->seaocore()->checkVersion($this->_version, '4.7.0'))) {
      if ($onlyPath)
  ...
  public function render($params = array()) {
    ...
    $getTheVersion = Engine_Api::_()->seaocore()->checkVersion($this->_version, '4.7.0');
    if (empty( $getTheVersion ) ) {
      return '
      if (typeof tinymce.editors[' . $params['element_id'] . '] == "object") {
        tinymce.editors[' . $params['element_id'] . '].destroy();
      }
      tinyMCE.init({
        mode: "exact",
      ...
      $getTheVersion = Engine_Api::_()->seaocore()->checkVersion($this->_version, '4.9.0');
      if( !empty( $getTheVersion ) ) {
      ...
      tinymce.init({
        ...
        toolbar2: "fontselect,fontsizeselect,bold,italic,underline,strikethrough,forecolor,backcolor,table,|,alignleft,aligncenter,alignright,alignjustify,|,outdent,indent,blockquote",
        toolbar3: "",
        element_format: "html",
        height: "225px",
        convert_urls: false,
