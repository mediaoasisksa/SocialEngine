...
class Seaocore_AuthController extends Core_Controller_Action_Standard {
  ...
  public function facebookAction() {
    ...
    $user_managed_groups = $this->_getParam('user_managed_groups', false);
    ...
    if ($user_managed_groups || isset($_SESSION['user_managed_groups'])) {
      $_SESSION['user_managed_groups'] = true;
      $permissions_array = array_merge($permissions_array, array(
        'user_managed_groups',
      ));
    }
...
//    }
    ...
    if (!empty($redirect_uri)) {
      $session->aaf_redirect_uri = $tempDecodeUrl = urldecode($this->_getParam('redirect_urimain'));

      if (!empty($fbfeedpublisher))
        $_SESSION['fbfeedpublisher'] = 1;
      else {
        $exploded_url = array();
        parse_str($tempDecodeUrl, $exploded_url);
        if (isset($exploded_url['fbfeedpublisher']) && !empty($exploded_url['fbfeedpublisher']))
          $_SESSION['fbfeedpublisher'] = $exploded_url['fbfeedpublisher'];
      }
    }
    // Already connected
    ...
    if ($facebook && $facebook->getUser() && empty($_GET['redirect_urimain'])) {
        ...
        if (!isset($_GET['redirect_fbback'])) {
          ...
          //  }
            ...
            if (!empty($read_stream) && !$facebookInviteApi->checkPermission('read_stream', $permissions)) {
              ...
              if (!$facebookInviteApi->checkPermission('user_managed_groups', $permissions)) {
      ...
      if ($enable_sitemobile && Engine_API::_()->sitemobile()->checkMode('mobile-mode')) {
        ...
        if (isset($_SESSION['user_managed_groups']))
          unset($_SESSION['user_managed_groups']);
...
//                die;
  ...
  public function linkedinAction() {
    ...
    $client_id = Engine_Api::_()->getApi('settings', 'core')->getSetting('linkedin.apikey');
    $client_secret = Engine_Api::_()->getApi('settings', 'core')->getSetting('linkedin.secretkey');
    $redirect_url = (_ENGINE_SSL ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . $this->view->url() ;
    $db = Engine_Db_Table::getDefaultAdapter();
    ...
    if (isset($_GET['code'])) {
      ...
      $url = Linkedin::_URL_ACCESS.'?' . http_build_query($params);
      ...
      if ($body->access_token) {
        $return_data['linkedin']['token'] = $body->access_token;
        $return_data['linkedin']['expire'] = $body->expires_in;
      }

      if($httpCode == 200) {
        // tokens retrieved
        $OBJ_linkedin->setToken($return_data['linkedin']);
        ...
        // set the response
        // $response            = $return_data;
        $response['error']   = 'HTTP response from LinkedIn end-point was not code 200';
        $response['success'] = FALSE;
      }

      if ($response['success'] == TRUE) {
        $_SESSION['linkedin_obj'] = $OBJ_linkedin;
        $_SESSION['linkedin_token2'] = $linkedin_token = $response['linkedin']['token'];
        // $_SESSION['linkedin_secret2'] = $linkedin_secret = $response['linkedin']['oauth_token_secret'];
        $_SESSION['linkedin_token'] = $response['linkedin']['token'];
        ...
        $getUserinfo = json_decode($getUserinfo['linkedin'], 1);
        ...
        if ($viewer->getIdentity() && isset($getUserinfo['id']) && (!$enable_sitemobile || !Engine_API::_()->sitemobile()->checkMode('mobile-mode'))) {
          ...
          if (!empty($info)) {
            ...
            $redirect_urimain = urlencode(( _ENGINE_SSL ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $URL_Home . '?redirect_linkedin=1');

            if (!empty($info['linkedin_uid']) && $info['linkedin_uid'] != $getUserinfo['id']) {
              $linkedinTable->update(array(
                'linkedin_uid' => $getUserinfo['id'],
                'linkedin_token' => $linkedin_token,
                'linkedin_secret' => $_GET['code'],
                ), array(
                'user_id = ?' => $viewer->getIdentity(),
              ));
          ...
          if (!empty($_GET['redirect_urimain'])) {
            ...
            return $this->_helper->redirector->gotoUrl($returnUrl, array('prependBase' => false));
          } elseif ($redirect_urimain && $_SESSION['linkedin_token2']) {
            //CHECK IF ALREADY THERE ARE SOME QUERY PARAMETERS OR NOT.
            $queryString = explode("?", urldecode($redirect_urimain));
            if (isset($queryString[1])) {
              $returnUrl = urldecode($redirect_urimain) . '&redirect_linkedin=1';
            } else {
              $returnUrl = $redirect_urimain . '?redirect_linkedin=1';
            }
        ...
        // if ($response['success'] === TRUE) {
          ...
          header('Location: ' . Linkedin::_URL_AUTH . http_build_query($params));
          die;
        // }
      }
    }
    ...
    if (!isset($_GET['redirect_linkedin'])) {
      ...
      $OBJ_linkedin->setCallbackUrl((_ENGINE_SSL ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . $this->view->url() . '?redirect_urimain=' . $redirect_uri);
      $OBJ_linkedin->setState(uniqid('', true));
      $OBJ_linkedin->setToken(NULL);
      // $response = $OBJ_linkedin->retrieveTokenRequest();
      // if ($response['success'] === TRUE) {
      //   // store the request token
  ...
  public function logoutAction() {
    ...
    if ($this->_getParam('logout_service') == 'facebook') {
      try {
        ...
        if (isset($session->fb_can_managegroups))
          unset($session->fb_can_managegroups);
...
//        }
        ...
        $OBJ_linkedin->setState($API_CONFIG['state']);
