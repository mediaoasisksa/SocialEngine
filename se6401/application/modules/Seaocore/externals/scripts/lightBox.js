      ...
      scriptJquery('.thumbs_photo').each(function(el){ 
        ...
        if(el.prop('onclick') != null)
          addEventClickFlag=false;
				...
				var	parentEl=el.parent('.generic_layout_container');
				  if(parentEl) {
    ...
    classname="."+classname;
      if(scriptJquery(classname)) {
				scriptJquery(classname).each(function(el) {       
					if(el.get('.thumb_profile')) {
						el.get('.thumb_profile').off('click').on('click', function(e) {
							e.stop();     
						...
						el.get('.thumb_normal').off('click').on('click', function(e) {
							e.stop();     
							href = openLightboxforActivityFeed(el);
							openSeaocoreLightBox(href);
						});
...
function openLightboxforActivityFeed(spanElement) {
  if(spanElement.get('.feed_item_thumb')) {
    href = spanElement.get('.feed_item_thumb');  
  } 
  else {
...
function openSeaocoreLightBox(href){
  ...
  en4.core.request.send(scriptJquery.ajax({
    ...
    success : function(responseHTML) {     
      ...
      if(scriptJquery('#community_sea_ads').length && scriptJquery('#lightbox_communityads_hidden').length){
        scriptJquery('#community_sea_ads').html( scriptJquery('#lightbox_communityads_hidden').html() );
        scriptJquery('#lightbox_communityads_hidden').html( '' );
      }
...
window.addEventListener('DOMContentLoaded', function(){
  if(document.getElementById('white_content_default_sea_lightbox')) {
    scriptJquery('#white_content_default_sea_lightbox').on('click', function(event) {
...
function photopaginationSocialenginealbum(url, modulename, tab_id) {
	if(document.getElementById('showlocation'))
		document.getElementById('showlocation').style.display = 'none';
	if(scriptJquery('#community_sea_ads').length && scriptJquery('#lightbox_communityads_hidden').length){
		//scriptJquery('#lightbox_communityads_hidden').innerHTML = scriptJquery('#community_sea_ads').innerHTML;
		lightbox_communityads_hidden = scriptJquery('#community_sea_ads').innerHTML;
		scriptJquery('#lightbox_communityads_hidden').html( '' );
	}
  ...
  en4.core.request.send(scriptJquery.ajax({
    ...
    success : function(responseHTML) {
      ...
      setImageScrollAlbum("auto");
      scriptJquery('#white_content_default_sea_lightbox').html( responseHTML );
      if(scriptJquery('#community_sea_ads').length){
        scriptJquery('#community_sea_ads').html( lightbox_communityads_hidden );
      }
...
var loadingImageSeaocore = function() {
  if(document.getElementById('media_image_div_seaocore'))
    scriptJquery('#media_photo').src = "application/modules/Seaocore/externals/images/icons/loader.gif";
  scriptJquery('#media_photo').style.marginTop='245px';
};
