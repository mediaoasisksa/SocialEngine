...
*/
var Slider = function(el, knob,bkg, options, maxknob){
	this.options = {
		onChange: function(){},
		onComplete: function(){},
		onTick: function(pos){
			this.moveKnob.css(this.p, pos);			
		},
		start: 0,
		end: 100,
		offset: 0,
    	...
    	this.initialize = function(el, knob,bkg, options, maxknob) { 
		this.options = scriptJquery.extend(this.options, options);
		this.element = scriptJquery(el);
		this.knob = scriptJquery(knob);
		this.previousChange = this.previousEnd = this.step = -1;
		this.bkg = scriptJquery(bkg);
		if(this.options.steps==null){
			this.options.steps = this.options.end - this.options.start;
		}
		if(maxknob!=null)
			this.maxknob = scriptJquery(maxknob);
		//else
		//	this.element.on('mousedown', this.clickedElement.bindWithEvent(this));
		var mod, offset;
		...
		this.max = this.element[0]['offsetWidth'] - this.knob[0]['offsetWidth'] + (this.options.offset * 2);
		this.half = this.knob[0]['offsetWidth']/2;
		this.full = this.element[0]['offsetWidth'] - this.knob[0]['offsetWidth'] + (this.options.offset * 2);
		this.min = this.$chk(this.options.range[0]) ? this.options.range[0] : 0;
		// this.getPos = this.element['get' + this.p.capitalize()].bind(this.element);
		this.knob.css('position', 'relative').css(this.p, - this.options.offset);
		...
		if(maxknob != null) {
			...
			this.maxknob.css('position', 'relative').css(this.p, + this.max - this.options.offset).css('bottom', 11);
		}
		var lim = {};
		//status = this.z
	...
	this.setMax = function(stepMax){
		...
		if(this.options.isinit){
			...
			mx= parseInt(this.maxknob.css('left')) - this.options.offset-4 ;
			lim[this.z] = [mi, mx];
	...
	this.clickedElement = function(event){
		...
		//this.moveKnob = this.knob;
		this.bkg.style.clip = "rect(0px "+  (parseInt(this.toPosition(this.step)) +3) + "px 8px 0px)"  
	...
	this.checkStep = function(mx){
		...
		if(this.maxknob!=null) {
			...
			mi = - this.options.offset; 
			//mx= parseInt(this.maxknob.css('left')) - this.options.offset-4 ;
            mx= parseInt(this.maxknob.css('left')) - this.options.offset+5 ;
			//mx= parseInt(this.maxknob.css('left')) - this.options.offset ;
			lim[this.z] = [mi, mx];
			this.drag.options.limit = lim;
		

			//mi = parseInt(this.knob.css('left'))-this.options.offset+22; 
            mi = parseInt(this.knob.css('left'))-this.options.offset+8; 
			//mi = parseInt(this.knob.css('left'))-this.options.offset; 
			...
			if(this.step < this.maxstep){
				...
				//this.clip_l = parseInt(this.knob.css('left'));
			}
			else{
				this.fireEvent('onChange', { minpos: this.maxstep, maxpos: this.step });
				//this.clip_l = (parseInt(this.maxknob.css('left')) + 10) ;
			}	
			this.clip_l = parseInt(this.knob.css('left')) + 8;
			//var w = Math.abs(parseInt(this.knob.css('left')) - parseInt(this.maxknob.css('left'))) + 3;	
			var w = Math.abs(parseInt(this.knob.css('left')) - parseInt(this.maxknob.css('left')));
			//if(w > 3) w = w+3;
			...
			this.bkg.style.clip = "rect(0px "+  (parseInt(this.drag.value.now[this.z]) +3)  + "px 8px 0px)"  
	...
	this.initialize(el, knob,bkg, options, maxknob);
