en4.core.runonce.add(function() {
  ...
  if( typeof flag != 'undefined' && flag != 0 ) {
    if(scriptJquery('.thumbs_photo').length) {
      scriptJquery('.thumbs_photo').each(function(temp,el){
        el = scriptJquery(el);
  ...
  if( typeof activityfeed_lightbox != 'undefined' && activityfeed_lightbox != 0 ) {
...
function addSEAOPhotoOpenEvent(classnames){
  classnames.forEach(function(classname) {    
    classname="."+classname;
    if(scriptJquery(classname).length) {
      scriptJquery(classname).each(function(temp,el) {
        el = scriptJquery(el);
        if(el.find('.thumb_profile')) {
          el.find('.thumb_profile').off('click').on('click', function(e) {
            e.stop();     
            href = openLightboxforActivityFeedHREF(el);
            openSeaocoreLightBox(href);
          });   
        } else {
          el.find('.thumb_normal').off('click').on('click', function(e) {
            e.stop();     
            href = openLightboxforActivityFeedHREF(el);
            openSeaocoreLightBox(href);
          });
...
function openSeaocoreLightBox(href){ 
  ...
  scrollPosition['top']=scriptJquery(window).scrollTop();
...
function getSEAOCorePhoto(href,isajax,params,imagepath){
  ...
  if(isajax==0){
    ...
    scriptJquery(".lightbox_btm_bl").each(function(i, el){
...
var closeSEAOLightBoxAlbum = function()
{
  if(fullScreenApi.isFullScreen()){
    ...
    loadedAllPhotos = '';
    scriptJquery('#seaocore_photo_lightbox').empty();
