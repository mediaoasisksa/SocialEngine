  ...
  en4.core.runonce.add(function() {
    ...
    scriptJquery('.comments_comment_likes').on('mouseover', function(event) {
      ...
      if( !el.data('tip-loaded') ) {
        ...
        // Load the likes
        scriptJquery.ajax({
          url : '<?php echo $this->url(array('module' => 'activity', 'controller' => 'index', 'action' => 'get-likes'), 'default', true) ?>',
          data : { format : 'json', action_id : el.closest('li').closest('li').closest('li').attr('id').attr(/\d+/)[0], comment_id : el.attr('id').attr(/\d+/)[0] },
