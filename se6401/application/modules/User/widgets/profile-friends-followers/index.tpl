...
<script type="text/javascript">
  var toggleFriendsPulldown = function(event, element, user_id) {
    ...
    if(scriptJquery.prop('tagName') != 'A') {
      return;
    }
    ...
    if(element.hasClass('pulldown').length) {
  ...
  en4.core.runonce.add(function(){
    ...
    var anchor = scriptJquery('#user_profile_friends_followers').parent();
    scriptJquery('#user_profile_friends_followers_previous').css("display",'<?php echo ( $this->friends->getCurrentPageNumber() == 1 ? 'none' : '' ) ?>');
    scriptJquery('#user_profile_friends_followers_next').css("display",'<?php echo ( $this->friends->count() == $this->friends->getCurrentPageNumber() ? 'none' : '' ) ?>');

    scriptJquery('#user_profile_friends_followers_previous').off('click').on('click', function(){
    ...
    scriptJquery('#user_profile_friends_followers_next').off('click').on('click', function(){
      en4.core.request.send(scriptJquery.ajax({
        url : en4.core.baseUrl + 'widget/index/content_id/' + <?php echo sprintf('%d', $this->identity) ?>,
        dataType: 'html',
        method : 'post',
  ...
  <?php foreach( $this->friends as $membership ):
    ...
    <li id="user_friend_<?php echo $member->getIdentity() ?>">
      ...
      <div class='profile_friends_body'>
        ...
        <div class='profile_friends_options profile_friends_followers'>
