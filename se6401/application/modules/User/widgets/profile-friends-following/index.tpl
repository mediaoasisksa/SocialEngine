...
<script type="text/javascript">
  var toggleFriendsPulldown = function(event, element, user_id) {
    if(scriptJquery(element).prop('tagName') != 'A' ) {
      return;
    }
    
    scriptJquery('.profile_friends_lists').each(function(otherElement) {
    ...
    if(scriptJquery(element).hasClass('pulldown') ) {
  ...
  var handleFriendList = function(event, element, user_id, list_id) {
    ...
    if( !scriptJquery(element).hasClass('friend_list_joined') ) {
      // Add
  ...
  var createFriendList = function(event, element, user_id) {
    ...
    request.complete(function(responseJSON) {
      if( responseJSON.status ) {
        var topRelEl = element.parent();
        scriptJquery('.profile_friends_lists ul').each(function(e) {
          let el = scriptJquery(this);
          var relEl = el.find('input').parent();
  ...
  en4.core.runonce.add(function(){
    ...
    var anchor = scriptJquery('#user_profile_friends_following').parent();
    scriptJquery('#user_profile_friends_following_previous').css("display",'<?php echo ( $this->friends->getCurrentPageNumber() == 1 ? 'none' : '' ) ?>');
    scriptJquery('#user_profile_friends_following_next').css("display",'<?php echo ( $this->friends->count() == $this->friends->getCurrentPageNumber() ? 'none' : '' ) ?>');

    scriptJquery('#user_profile_friends_following_previous').off('click').on('click', function(){
    ...
    scriptJquery('#user_profile_friends_following_next').off('click').on('click', function(){
      en4.core.request.send(scriptJquery.ajax({
        url : en4.core.baseUrl + 'widget/index/content_id/' + <?php echo sprintf('%d', $this->identity) ?>,
        dataType: 'html',
        method : 'post',
    ...
    scriptJquery('.friends_lists_menu_input input').each(function(e){
      scriptJquery(this).on('blur', function() {
        scriptJquery(this).parent('.drop_down_frame')[0].css("visibility","hidden");
      });
    });
  });
</script>
              ...
              <div class="pulldown_contents_wrapper">
                <div class="pulldown_contents">
                  <ul>
                    ...
                    <?php endforeach; ?>
                    <li>
                      <input type="text" title="<?php echo $this->translate('New list...') ?>" onclick="new Event(event).preventDefault();" onkeypress="if( new Event(event).key == 'enter' ) { createFriendList(event, scriptJquery(this), '<?php echo $member->user_id ?>'); }" />
                    </li>
                  </ul>
                </div>
              </div>
