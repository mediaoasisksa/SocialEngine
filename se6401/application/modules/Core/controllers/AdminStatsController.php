 ...
 * @version    $Id: AdminStatsController.php 9812 2012-11-01 02:14:01Z matthew $
 * @author     Jung
 */
...
class Core_AdminStatsController extends Core_Controller_Action_Admin
{
  ...
  public function referrersAction()
  {
    ...
    $select = $table->select()
      ->order('value DESC')
      ->limit(100);
    $this->view->referrers = $table->fetchAll($select);
  }
  ...
  public function chartDataAction()
  {
    ...
    if( !$chunk || !engine_in_array($chunk, $this->_periods) ) {
      $chunk = Zend_Date::DAY;
    }
    if( !$period || !engine_in_array($period, $this->_periods) ) {
