...
class Sitecourse_Widget_CourseCarouselController extends Seaocore_Content_Widget_Abstract {

    public function indexAction(){
        $itemCount = $this->_getParam('itemCount',2);
        $courseType = $this->_getParam('courseType',1);
        $sortingCriteria = $this->_getParam('sortingCriteria',1);
        $courseInfo = $this->_getParam('courseInfo');
        $this->view->tabItem = $this->_getParam('tabletItemCount',2);
        $this->view->deskItem = $this->_getParam('desktopItemCount',2);
        $this->view->carouselSpeed = $this->_getParam('carouselSpeed',3000);
        $title = $this->_getParam('title');

        $courses = Engine_Api::_()->getDbtable('courses','sitecourse')->getCarouselCourses($itemCount,$courseType,$sortingCriteria);
        // no course found
        if(empty($courses)){
