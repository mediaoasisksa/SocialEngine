...
<script type="text/javascript"> 
  function addMessage(message) {
    if(scriptJquery('#seao_message').length) {
      scriptJquery('#seao_message').html(message);
    } else {
      scriptJquery('.form-elements').prepend(scriptJquery.crtEle('span', {'id': 'seao_message', 'style': 'color: red'}).text(message));
    }
  ...
  window.callbackOnGetResponseOfSEAOLicenses = function (data) {
    if (data.status == 1) {
      scriptJquery.ajax({
        ...
        dataType: 'json',
        method: 'post',
        data: scriptJquery.extend(data, { format: 'json' }),
