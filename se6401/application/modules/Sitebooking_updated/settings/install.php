...
class Sitebooking_Installer extends Sitecore_License_Installer
{
 ...
 public function onInstall()
 {
  ...
  $this->_dashboardMenusAndroidWork();
  $this->_dashboardMenusIOSWork();
...
protected function _dashboardMenusAndroidWork() {

  $db = $this->getDb();            
       
  // CHECK THAT ANDROID PLUGIN IS INSTALLED OR NOT
  $select = new Zend_Db_Select($db);
  $select
  ...
  if(!$isAndroidModuleEnabled)
      return;

  $select = new Zend_Db_Select($db);
  $isSitebookingRowExist = $select->from('engine4_siteandroidapp_menus')
            ->where('name = ?', "core_main_provider")
            ->limit(1)
            ->query()
            ->fetchColumn();

  if (empty($isSitebookingRowExist)&& $isAndroidModuleEnabled) {
      $schema = 'http://';
      ...
      $appointmentUrl = (!empty($baseParentUrl)) ? $getHost . DIRECTORY_SEPARATOR . $baseParentUrl . DIRECTORY_SEPARATOR . 'bookings/booked/services' : $getHost . DIRECTORY_SEPARATOR . 'bookings/booked/services';

      $db->query('INSERT INTO `engine4_siteandroidapp_menus` (`name`, `dashboard_label`, `header_label`, `module`, `icon`, `url`, `show`, `type`, `status`, `default`, `order`) VALUES ("service_wishlist", "Service Wishlist", "Service Wishlist", "sitebooking", "", "", "login", "menu", "1", "0", "15")');

      $db->query('INSERT INTO `engine4_siteandroidapp_menus` (`name`, `dashboard_label`, `header_label`, `module`, `icon`, `url`, `show`, `type`, `status`, `default`, `order`) VALUES ("provider_wishlist", "Provider Wishlist", "Provider Wishlist", "sitebooking", "", "", "login", "menu", "1", "0", "15")');

      $db->query('INSERT INTO `engine4_siteandroidapp_menus` (`name`, `dashboard_label`, `header_label`, `module`, `icon`, `url`, `show`, `type`, `status`, `default`, `order`) VALUES (NULL, "My Appointments", "My Appointments", NULL, "", "'. $appointmentUrl . '", "login", "menu", "1", "0", "15")');

      $db->query('INSERT INTO `engine4_siteandroidapp_menus` (`name`, `dashboard_label`, `header_label`, `module`, `icon`, `url`, `show`, `type`, `status`, `default`, `order`) VALUES (NULL, "Service Bookings", NULL, NULL, "", "", "both", "category", "1", "0", "12")');

      $db->query('INSERT INTO `engine4_siteandroidapp_menus` (`name`, `dashboard_label`, `header_label`, `module`, `icon`, `url`, `show`, `type`, `status`, `default`, `order`) VALUES ("core_main_services", "Services", "Services", "sitebooking", NULL, NULL, "both", "menu", "1", "0", "14")');

      $db->query('INSERT INTO `engine4_siteandroidapp_menus` (`name`, `dashboard_label`, `header_label`, `module`, `icon`, `url`, `show`, `type`, `status`, `default`, `order`) VALUES ("core_main_provider", "Providers", "Providers", "sitebooking", "", "", "both", "menu", "1", "0", "13")');
  }
}

protected function _dashboardMenusIOSWork() {

  $db = $this->getDb();
       
  // CHECK THAT IOS PLUGIN IS INSTALLED OR NOT
  $select = new Zend_Db_Select($db);
  $select
          ->from('engine4_core_modules')
  ...
  if(!$isIOSModuleEnabled)
      return;

  $select = new Zend_Db_Select($db);
  $isSitebookingRowExist = $select->from('engine4_siteiosapp_menus')
          ->where('name = ?', "core_main_provider")
          ->limit(1)
          ->query()
          ->fetchColumn();

  if (empty($isSitebookingRowExist)&& $isIOSModuleEnabled) {
    $schema = 'http://';
    if (!empty($_SERVER["HTTPS"]) && 'on' == strtolower($_SERVER["HTTPS"])) {
        $schema = 'https://';
    }
    ...
    $db->query('INSERT INTO `engine4_siteiosapp_menus` (`name`, `dashboard_label`, `header_label`, `module`, `icon`, `url`, `show`, `type`, `status`, `default`, `order`) VALUES ("service_wishlist", "Service Wishlist", "Service Wishlist", "sitebooking", "", "", "login", "menu", "1", "0", "45")');

    $db->query('INSERT INTO `engine4_siteiosapp_menus` (`name`, `dashboard_label`, `header_label`, `module`, `icon`, `url`, `show`, `type`, `status`, `default`, `order`) VALUES ("provider_wishlist", "Provider Wishlist", "Provider Wishlist", "sitebooking", "", "", "login", "menu", "1", "0", "43")');

    $db->query('INSERT INTO `engine4_siteiosapp_menus` (`name`, `dashboard_label`, `header_label`, `module`, `icon`, `url`, `show`, `type`, `status`, `default`, `order`) VALUES (NULL, "My Appointments", "My Appointments", NULL, "", "'. $appointmentUrl . '", "login", "menu", "1", "0", "46")');

    $db->query('INSERT INTO `engine4_siteiosapp_menus` (`name`, `dashboard_label`, `header_label`, `module`, `icon`, `url`, `show`, `type`, `status`, `default`, `order`) VALUES (NULL, "Service Bookings", NULL, NULL, "", "", "both", "category", "1", "0", "41")');

    $db->query('INSERT INTO `engine4_siteiosapp_menus` (`name`, `dashboard_label`, `header_label`, `module`, `icon`, `url`, `show`, `type`, `status`, `default`, `order`) VALUES ("core_main_services", "Services", "Services", "sitebooking", NULL, NULL, "both", "menu", "1", "0", "44")');

    $db->query('INSERT INTO `engine4_siteiosapp_menus` (`name`, `dashboard_label`, `header_label`, `module`, `icon`, `url`, `show`, `type`, `status`, `default`, `order`) VALUES ("core_main_provider", "Providers", "Providers", "sitebooking", "", "", "both", "menu", "1", "0", "42")');
  }
}
