...
class Sitebooking_AdminProviderOfTheDayController extends Core_Controller_Action_Admin {
  ...
  public function dayAction() {
    ...
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')
                ->getNavigation('sitebooking_admin_main', array(), 'sitebooking_admin_main_provideroftheday');
    ...
    $this->view->assign($values);
		...
		//FETCH DATA
    $this->view->paginator = $paginator = Engine_Api::_()->getDbtable('itemofthedays', 'sitebooking')->getProviderOfDayList($values, 'pro_id', 'sitebooking_pro');
  ...
  public function addItemAction() {
    ...
    //FORM GENERATION
    $form = $this->view->form = new Sitebooking_Form_Admin_ProviderOfTheDay_Provideroftheday();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      try {
        ...
        $select = $table->select()->where('resource_id = ?', $values["resource_id"])->where('resource_type = ?', 'sitebooking_pro');
        ...
        $row->end_date = $values["endtime"];
				$row->resource_type = 'sitebooking_pro';
        $row->save();
      ...
      $this->_forward('success', 'utility', 'core', array(
              'smoothboxClose' => true,
              'parentRefresh' => true,
              'messages' => array(Zend_Registry::get('Zend_Translate')->_('The Provider of the Day has been added successfully.'))
      ));
    }
  }
  ...
  public function getitemAction() {
    ...
    $data = array();
		...
		$moduleProviders = Engine_Api::_()->getItemTable('sitebooking_pro')->getDayItems($search_text, $limit=10);
    foreach ($moduleProviders as $moduleProvider) {

			$content_photo = $this->view->itemPhoto($moduleProvider, 'thumb.icon');

      $data[] = array(
              'id' => $moduleProvider->pro_id,
              'label' => $moduleProvider->title,
              'photo' => $content_photo
      );
    }
  ...
  public function deleteItemAction() {
    ...
    $this->renderScript('admin-provider-of-the-day/delete.tpl');
  }
