...
function tellAFriendAction()
{
  ...
  function availableAction()
  {
    ...
    if(!empty( $_POST['save'] )){
      $this->view->ser_id = $values['ser_id'] = $_POST['ser_id'];
  ...
  public function bookedAction(){
    ...
    if($this->_getparam('isAjax')){
      ...
      Engine_Api::_()->sitebooking()->sendServiceAcceptMail($user,$serviceItem,$owner);
        // schedule work
        if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('calendar')) {
          Engine_Api::_()->getDbtable('schedules', 'calendar')->setBookingSchedules($bookingItem, $bookingItem->duration);
        }
    ...
    if(!empty($_POST['find']) ){
      if(!empty($_POST['servicing_date']['date'])){
        $servicing_date = str_replace("/","-",$_POST['servicing_date']['date']);
        $date1 = strtotime($servicing_date);
        $servicing_date = date('Y-m-d', $date1);
        $_POST['servicing_date'] = $servicing_date;
      }else{
        $_POST['servicing_date'] = '';
      }
      if(!empty($_POST['booking_date']['date'])){
        $bookingdate = str_replace("/","-",$_POST['booking_date']['date']);
        $date2 = strtotime($bookingdate);
        $bookingdate = date('Y-m-d', $date2);
        $_POST['booking_date'] = $bookingdate;
      }else{
        $_POST['booking_date'] = '';
      }
