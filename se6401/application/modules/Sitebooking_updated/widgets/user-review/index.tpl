...
<script type="text/javascript">
  ...
  for(var x=1; x<=parseInt(rating); x++) {
    scriptJquery('#my_rate_'+x).attr('class', 'rating_star_big_generic rating_star_big');
  }

  var remainder = Math.round(rating)-rating;

  for(var x=parseInt(rating)+1; x<=5; x++) {
    scriptJquery('#my_rate_'+x).attr('class', 'rating_star_big_generic rating_star_big_disabled');
  }
  
  if (remainder <= 0.5 && remainder !=0){
    var last = parseInt(rating)+1;
    scriptJquery('#my_rate_'+last).attr('class', 'rating_star_big_generic rating_star_big_half');
  }
</script>
...
<script type="text/javascript">
  ...
  window.addEventListener('DOMContentLoaded', function(){
    ...
    scriptJquery('#rating_text').innerHTML = " <?php echo $this->translate(array('%s rating', '%s ratings', $this->rating_count),$this->locale()->toNumber($this->rating_count)) ?>";

    var rating = "<?php echo $this->avgRating;?>";
    
    for(var x=1; x<=parseInt(rating); x++) {
      scriptJquery('#avg_rate_'+x).attr('class', 'rating_star_big_generic rating_star_big');
    }

    var remainder = Math.round(rating)-rating;

    for(var x=parseInt(rating)+1; x<=5; x++) {
      scriptJquery('#avg_rate_'+x).attr('class', 'rating_star_big_generic rating_star_big_disabled');
    }
    
    if (remainder <= 0.5 && remainder !=0){
      var last = parseInt(rating)+1;
      scriptJquery('#avg_rate_'+last).attr('class', 'rating_star_big_generic rating_star_big_half');
    }
      ...
      <script type="text/javascript">
        en4.core.runonce.add( function() {
          ...
          for(var x=1; x<=parseInt(rating); x++) {
            ...
            document.getElementById(id).attr('class', 'rating_star_big_generic rating_star_big');
          }
          ...
          for(var x=parseInt(rating)+1; x<=5; x++) {
            ...
            var id = <?php echo $item->getIdentity() ?>;
            id = "user_rate_"+id+"_"+x;
            document.getElementById(id).attr('class', 'rating_star_big_generic rating_star_big_disabled');
          }
          ...
          if (remainder <= 0.5 && remainder !=0){
            ...
            document.getElementById(id).attr('class', 'rating_star_big_generic rating_star_big_half');
          }
        });
      </script>
...
<script type="text/javascript">
  scriptJquery('#view_more_<?php echo $this->identity;?>').on('click', function(){
    ...
    en4.core.request.send(scriptJquery.ajax({
      ...
      scriptJquery('#user_reviews').innerHTML = scriptJquery('#user_reviews').innerHTML + responseHTML; 
