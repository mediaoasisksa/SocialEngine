...
<script type="text/javascript">
  ...
  en4.core.request.send(new Request.HTML({
    ...
    onRequest: function () {
    $('loader_<?php echo $this->identity ?>').show();
    }, 
    onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
    $('loader_<?php echo $this->identity ?>').hide();
    document.getElementById("provider_services_<?php echo $this->identity ?>").innerHTML = document.getElementById("provider_services_<?php echo $this->identity ?>").innerHTML + responseHTML;
    }     
  }));
  var cpage = <?php echo sprintf('%d', $this->paginator->getCurrentPageNumber()) ?>;
  var pages = <?php echo $this->paginator->count() ?>;
  if(pages <= cpage+1) {
    var element = document.getElementById("services_view_more_<?php echo $this->identity ?>");
