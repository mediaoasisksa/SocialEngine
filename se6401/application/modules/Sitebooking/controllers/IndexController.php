...
class Sitebooking_IndexController extends Core_Controller_Action_Standard
{
  ...
  public function bookServiceAction(){
    ...
    $this->view->viewrTimezone = $viewer->getIdentity() ? $viewer->timezone: 'Europe/Moscow';
    ...
    if($scheduleRow){
      ...
      foreach ($data as $key => $value) {
        ...
        $date2 = date_timezone_set($date1, timezone_open($this->view->viewrTimezone));
    ...
    $this->view->timeSlot = $timeSlot;
     ...
     if( $_GET['date']) {
            $p['user_id'] = $values['user_id'];
          $p['ser_id'] = $values['ser_id'];
          $p['servicing_date'] = $_GET['date'];
          
          $hasBooking = $bookingTable->hasBooking($p);
          if($hasBooking) {
             $this->view->message = "You have already book a slot for this day for this service. please go back and choose other day.";
      ...
      try {
        $servDuration = Engine_Api::_()->sitebooking()->timezoneConvertUsingDuration($this->view->viewrTimezone,'UTC',$_POST['duration'],$duration);
        $servicingDate = '';
        ...
        $p['servicing_date'] = $_POST['servicing_date'];
        
        if($_POST['servicing_end_date'] && !empty($_POST['servicing_end_date'])) {
            $values['servicing_end_date'] = date("Y-m-d", strtotime($_POST['servicing_date'] . ' +30 days'));
        }
        ...
        $values = array();
        $values['user_id'] = $bookingTableRow->user_id;
        $values['gateway_id'] = 2;
        $values['price'] = $bookingTableRow->total_charges;
        $values['timezone'] = $_POST['timezone'];
        $values['duration'] = $this->view->duration / 60;
        $values['currency'] = 'SAR';
        $values['status'] = 'pending';
        $values['source_type'] = 'sitebooking_servicebooking';
        $values['source_id'] = $bookingTableRow->getIdentity();
        
        $bookingOrderTable = Engine_Api::_()->getItemTable('sitebooking_order');
        $bookingOrderTableRow = $bookingOrderTable->createRow();
        $bookingOrderTableRow->setFromArray($values);
        $id = $bookingOrderTableRow->save();
        
    	$session = new Zend_Session_Namespace('Sitebooking_PayPackage');
    	$session->user_id = $bookingTableRow->user_id;
    	$session->order_id = $id;
        $card = $_POST['sitebooking_payment'];
        $db->commit();
        $this->_helper->redirector->gotoUrl('https://pay.consl2.com/consl2-test?id='.$id.'&card='.$card.'&price='. $values['price'], array('prependBase' => false));
