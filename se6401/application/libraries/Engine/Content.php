...
class Engine_Content
{
        ...
        if (!isset($this->_loaders[$type])) {
            switch ($type) {
                ...
                case self::ELEMENT:
                    ...
                    break;
                default:
                    throw new Engine_Content_Exception(
    ...
    public function loadWidget($widget)
    {
        ...
        if (count($segments) == 2) {
            ...
            throw new Engine_Content_Exception(
                sprintf('Widget name must have exactly one or two sections, given "%s"', $widget)
            );
        }
        ...
        if (null !== $module && !Engine_Api::_()->hasModuleBootstrap($module)) {
            return false;
            throw new Engine_Content_Exception(
                sprintf('Widget\'s module is not enabled or not installed: "%s"', $module)
            );
        }
        ...
        if (!class_exists($class, false)) {
            ...
            if (!class_exists($class, false)) {
                throw new Engine_Content_Exception(
                    sprintf('Unable to load widget class "%s" in path "%s"', $class, $path)
                );
            }
        }

        /**
         * @var Engine_Content_Widget_Abstract $instance
         */
        $instance = new $class();
