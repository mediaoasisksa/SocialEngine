Lasso.Crop = new Class({
  ...
  initialize : function(img,options){
    ...
    if(sesJqueryObject('#ses_media_lightbox_container').length)
      this.img = sesJqueryObject('#gallery-img').get(0);
    var coords = this.img.getCoordinates();
    var widthFix = coords.width;
    var heightFix = coords.height;
    ...
    options.p = this.container;
		var urlimageSrc = this.img.style.backgroundImage;
		var imageSrc = urlimageSrc.slice(4, -1);
		imageSrc = imageSrc.replace(/"/g,'');
		if(!imageSrc){
		 imageSrc = this.img.get('src');
		}
