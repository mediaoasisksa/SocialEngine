...
var Tagger = new Class({
  ...
  initialize : function(el, options) {
    el = $(el);
    if(el && el.get('tag') != 'img' ) {
      this.image = el.getElement('img');
    } else {
      this.image = el;
    }
    if(sesJqueryObject('#ses_media_lightbox_container').length){
      if(sesJqueryObject('.image_show_pswp').parent().attr('id') == 'media_photo_next_ses'){
      el = sesJqueryObject('#media_photo_next_ses').get(0);
      }     
    }
    this.element = el;
    this.count = 0;
    ...
    this.setOptions(options);
		...
		// get  original image for container height and width for responsive work
		this.image_original = new Image();
    this.image_original.src = this.image.get('src'); 
  ...
  addTag : function(params) {
    ...
    ["x", "y", "w", "h"].each(function(key) {
      params.extra[key] = parseInt(params.extra[key]);
		...
		var getOriginalImageSize = this.getOriginalImageSize();
		var getCurrentImageContainerSize= this.getCurrentImageContainerSize();
		// check for x,y coordinates and fix for full page view.
		// for width work
		["x","w"].each(function(key){
				 params.extra[key]= (params.extra[key]/ getOriginalImageSize.width * getCurrentImageContainerSize.width).toInt();
		});
		
		// for height work
		["y" ,"h"].each(function(key){
				 params.extra[key]= (params.extra[key] / getOriginalImageSize.height * getCurrentImageContainerSize.height).toInt();
		});
    ...
    if( this.element.getSize().y < labelPos.top.toInt() + 20 && !$('media_photo_next_ses') ){
  ...
  createTag : function(params) {
    if( !this.options.enableCreate ) return;
		...
		// change params.extra array if required as per screen resolutions		
		params.extra = this.checkBeforeSave(params.extra);
  ...
  checkCanRemove : function(id) {
	...
	cropCoordinates: function(){
		// get coordinates from getCrop().
  ...
  hideTag : function(id) {
	...
	getOriginalImageSize:function(){
		 return {'width' : (this.image_original.width), 'height' : (this.image_original.height)};	
	},
	getCurrentImageContainerSize:function(){
		// image coordinates for adaptive height and width.
		var imageCordinates = this.image.getCoordinates();    
    return {'width' : (imageCordinates.width),'height' : (imageCordinates.height)}
	},
	  checkBeforeSave: function(params){
		// get original image coordinates
    var getOriginalImageSize =this.getOriginalImageSize();
		// get crop coordinates to check with original
    var coordinates =this.cropCoordinates();
    //change horizontal coordinates   
    ["x", "w"].each(function(key) {params[key]= ((params[key]/ coordinates.width) * getOriginalImageSize.width).toInt();});
    // change vertical coordinates   
    ["y", "h"].each(function(key) {params[key]= ((params[key] / coordinates.height) * getOriginalImageSize.height).toInt();});  
		// return params for save 
    return params;
  }
});