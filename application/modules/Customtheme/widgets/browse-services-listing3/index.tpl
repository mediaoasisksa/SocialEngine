...
<section class="custom_service_listing_container">
  <div class="enwan-constants">
    <h2><?php echo $this->translate("Get instant 1:1 help from expert consultant");?></h2>
    <div class="khat-cons"></div>
    <p style="font-size: 22px;font-weight: 300;width: 900px;margin: 18px auto;"><?php echo $this->translate("A quick, easy call with an expert is just one click away with our one-to-one sessions and get the knowledge you need to pursue your dream career");?></p>
  </div>
      ...
      <div class="custom_service_listing_item custom_service_listing_item_<?php echo $itemkey;?>">
        <article>
          <div class="custom_service_listing_item_left">
            <div class="itemthumb">
              <?php $itemkey++; $url = $item->getOwner()->getPhotoUrl('thumb.profile'); $url = $url ?  $url : '/application/themes/sescompany/images/nophoto_user_thumb_profile.png'?>
                            ...
                            <?php if($date1 < $date2 ): ?>
                              ...
                              $endTime = date_format($date3, 'H:i');
                                ...
                                $select = $tableBooking->select()
                                        ...
                                        ->where($bookingTableName . '.ser_id =?', $item->getIdentity())
                                        ->where($bookingTableName . '.servicing_date =?', $date);
                                $results = $tableBooking->fetchAll($select); 
                                
                                // //echo $item->getIdentity();die;
                                $durationTime = array();
                                foreach($results as $result) {
                                    $duration1 = json_decode($result['duration'], true);
                                    if(isset($duration1[$date])) {
                                        $servDuration = Engine_Api::_()->sitebooking()->timezoneConvert('UTC',$this->viewrTimezone,$result['duration']);
                                        
                                        //$d = Engine_Api::_()->sitebooking()->timezoneConvert($this->viewrTimezone,$this->viewrTimezone,$result['duration']);
                                        
                                        $durationTime[] = $servDuration[$date];
                                        // $start = strtotime($date.$duration[$date]);
                                        // $end = strtotime($values['endtime']);
                                        // $durationTime[] = $duration[$date];
                                    }
                                }
                              ?>
                              
                              <?php if(in_array(date($timeFormateForUI, strtotime($value)). '-' . $endTime, $durationTime)):?>
                                  <div class="item_slots">
                                    <span class="disabled" style="    color: darkgray;
    cursor: not-allowed;
    opacity: 0.5;
    text-decoration: none;"><b><?php echo date($timeFormateForUI, strtotime($value))?></b></span>
                                  </div>
                              <?php else: ?>
                                    <div class="item_slots">
              ...
              <?php endif;?>
              <div class="itembtn" style="display:none;"> 
                <?php if(count($data) > 0 && $viewer->getIdentity() != $item->owner_id): ?>
                  <button onclick="window.location.href='bookings/service/book-service/<?php echo $item->getIdentity();?>'" class="custom_btn custom_btn_sec custom_btn_rounded"><?php echo $this->translate('Book Me');?></button>
                <?php elseif($viewer->getIdentity() == $item->owner_id):?>
                  <button onclick="window.location.href='/bookings/providers/<?php echo $parent_id;?>/services/service-manage'" class="custom_btn custom_btn_sec custom_btn_rounded">
...
customjqueryslick('.slider-nav-<?php echo $itemkey;?>').slick({
  ...
  responsive: [
    ...
    <?php endforeach;?>
      <?php else:?>
        <div class="tip">
            <span style="margin-left:12px;"><?php echo $this->translate("There is no consulants found.");?></span>
        </div>
    <?php endif;?>
  </div>        
</section>