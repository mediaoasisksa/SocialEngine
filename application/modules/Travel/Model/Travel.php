 ...
 * @version    $Id: Travel.php 9747 2012-07-26 02:08:08Z john $
 * @author     Donna
 */
 ...
 * @license    https://socialengine.com/eula
 */
class Travel_Model_Travel extends Core_Model_Item_Abstract
{
  // Properties
  ...
  public function setPhoto($photo)
  {
    if( $photo instanceof Zend_Form_Element_File ) {
      ...
      throw new Travel_Model_Exception('invalid argument passed to setPhoto');
    }
    ...
    $travelAlbum = $this->getSingletonAlbum();
    $photoItem = $photoTable->createRow();
    $photoItem->setFromArray(array(
      'travel_id' => $this->getIdentity(),
      'album_id' => $travelAlbum->getIdentity(),
      'user_id' => $viewer->getIdentity(),
      'file_id' => $iMain->getIdentity(),
      'collection_id' => $travelAlbum->getIdentity(),
    ));
    $photoItem->save();
  ...
  public function addPhoto($file_id){
    ...
    $photo = Engine_Api::_()->getDbtable('photos', 'travel')->createRow();
  ...
  {
    $table = Engine_Api::_()->getItemTable('travel_album');
    $select = $table->select()
      ->where('travel_id = ?', $this->getIdentity())
      ->order('album_id ASC')
      ->limit(1);
  ...
  public function getCategoryItem()
  {
      if(!$this->category_id)
        return false;
      return Engine_Api::_()->getItem('travel_category',$this->category_id);
  }
