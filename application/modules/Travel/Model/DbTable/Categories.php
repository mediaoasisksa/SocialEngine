 ...
 * @license    https://socialengine.com/eula
 */
class Travel_Model_DbTable_Categories extends Engine_Db_Table
{
  protected $_rowClass = 'Travel_Model_Category';
  ...
  public function getUserCategoriesAssoc($user)
  {
    ...
    $stmt = $this->getAdapter()
        ->select()
        ->from('engine4_travel_categories', array('category_id', 'category_name'))
        ->joinLeft('engine4_travel_travels', "engine4_travel_travels.category_id = engine4_travel_categories.category_id")
        ->group("engine4_travel_categories.category_id")
        ->where('engine4_travel_travels.owner_id = ?', $user)
        ->where('engine4_travel_travels.draft = ?', "0")
        ->order('category_name ASC')
        ->query();
