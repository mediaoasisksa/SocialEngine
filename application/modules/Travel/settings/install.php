 ...
 * @license    https://socialengine.com/eula
 */
class Travel_Installer extends Engine_Package_Installer_Module
{
    ...
    protected function _addHashtagSearchContent()
    {
        ...
        $select
            ...
            ->where('type = ?', 'widget')
            ->where('name = ?', 'travel.hashtag-search-results')
        ;
        ...
        if( empty($info) ) {
            ...
            $db->insert('engine4_core_content', array(
                ...
                'order'   => 200,
                'params'  => '{"title":"Travel Listings","titleCount":true}',
            ));
        }
    }
    ...
    protected function _addManagePage()
    {
        ...
        $pageId = $db->select()
            ->from('engine4_core_pages', 'page_id')
            ->where('name = ?', 'travel_index_manage')
            ->limit(1)
            ->query()
            ->fetchColumn();
        ...
        if( !$pageId ) {
            ...
            $db->insert('engine4_core_pages', array(
                'name' => 'travel_index_manage',
                'displayname' => 'Travel Manage Page',
                'title' => 'My Listings',
                'description' => 'This page lists a user\'s travel listings.',
                'custom' => 0,
            ));
    ...
    protected function _addCreatePage()
    {
        ...
        $pageId = $db->select()
            ->from('engine4_core_pages', 'page_id')
            ->where('name = ?', 'travel_index_create')
            ->limit(1)
            ->query()
            ->fetchColumn();
        ...
        if( !$pageId ) {
            ...
            // Insert page
            $db->insert('engine4_core_pages', array(
                'name' => 'travel_index_create',
                'displayname' => 'Travel Create Page',
                'title' => 'Post a New Listing',
                'description' => 'This page is the travel create page.',
                'custom' => 0,
            ));
            $pageId = $db->lastInsertId();
    ...
    protected function _addBrowsePage()
    {
        ...
        if( !$pageId ) {
            // Insert page
            $db->insert('engine4_core_pages', array(
                'name' => 'travel_index_index',
                'displayname' => 'Travel Browse Page',
                'title' => 'Travel Browse',
                'description' => 'This page lists travel listings.',
                'custom' => 0,
            ));
            $pageId = $db->lastInsertId();
            ...
            $db->insert('engine4_core_banners', array(
                ...
                'body' => 'Find exciting adventures or post your own Travel Listing!',
                'photo_id' => 0,
                'params' => '{"label":"Post a New Listing","route":"travel_general","routeParams":{"action":"create"}}',
                'custom' => 0
            ));
    ...
    protected function _addViewPage()
    {
        ...
        if( !$pageId ) {
            $db->insert('engine4_core_pages', array(
                'name' => 'travel_index_view',
                'displayname' => 'Travel View Page',
                'title' => 'View Travel',
                'description' => 'This is the view page for a travel.',
                'custom' => 0,
                'provides' => 'subject=travel',
            ));
    ...
    protected function _addUserProfileContent()
    {
        ...
        $select
            ...
            ->where('page_id = ?', $pageId)
            ->where('type = ?', 'widget')
            ->where('name = ?', 'travel.profile-travels')
        ;
        $info = $select->query()->fetch();
        if( empty($info) ) {
    ...
    protected function _addPrivacyColumn()
    {
        ...
        $db = $this->getDb();
        $sql = "ALTER TABLE `engine4_travel_travels` ADD `view_privacy` VARCHAR(24) NOT NULL DEFAULT 'owner' AFTER `closed`";
        try {
            $db->query($sql);
        } catch( Exception $e ) {
        ...
        foreach( $privacyList as $privacy ) {
            ...
            $db->update('engine4_travel_travels',array(
                'view_privacy' => $viewPrivacy,
            ), array(
                'travel_id = ?' => $privacy['resource_id'],
            ));
        }
