...
<script type="text/javascript">
  ...
  function addMessage(message) {
    scriptJquery('#seao_user_auth_details').prepend(scriptJquery.crtEle('span', {'id': 'seao_message', 'text': message, 'style': 'color: red'}));
  } 
  function installSelectedPlugins() {
    ...
    if (ready == true) {
      ...
      item = scriptJquery(elements[selectedItemLength]);
      sendInstallRequest(item.attr('data-url'), item.id);
    }
  }
