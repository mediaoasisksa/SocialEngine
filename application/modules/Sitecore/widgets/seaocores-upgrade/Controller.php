...
//    }
    //Delete SocialApps.tech modules folder message.
...
//    }
    ...
    $enabled = 1;
    $pluginInfoUrls = array('https://socialapps.tech/plugins/feed');
    if( $type == 'disabled' ) {
      $enabled = 0;
      $pluginInfoUrls = array('https://socialapps.tech/plugins/feed',
        'https://socialapps.tech/groupextensions/feed',
        'https://socialapps.tech/bizextensions/feed',
        'https://socialapps.tech/eventextensions/feed',
        'https://socialapps.tech/extensions/feed',
        'https://socialapps.tech/reviewextensions/feed',
        'https://socialapps.tech/themes/feed');
    }
    //Zend_Feed::setHttpClient(new Zend_Http_Client(null, array('timeout' => 60, 'adapter' => 'Zend_Http_Client_Adapter_Curl')));
    $rss = Zend_Feed::import('https://socialapps.tech/plugins/feed'); //THIS IS JUST TO TAKE COMMON TITLE AND DESC. 
    $channel = array(
      'title' => $rss->title(),
      'link' => $rss->link(),
    ...
    foreach( $pluginInfoUrls as $url ) {
      ...
      foreach( $rss as $item ) {
        ...
        if( strstr($product_type, 'sitevideo') ) {
          ...
          $sitevideoIntegration['link'] = 'https://socialapps.tech/videoextensions/socialengine-advanced-videos-pages-businesses-groups-listings-events-stores-extension';
        ...
        if( strstr($product_type, 'document') ) {
          ...
          $documentintegration['link'] = 'https://socialapps.tech/documentextensions/socialengine-documents-sharing-pages-businesses-groups-listings-events-stores-extension';
