 ...
 * @version    $Id: index.tpl 2010-11-18 9:40:21Z SocialApps.tech $
...
<h3>
	<?php echo $this->translate("SocialApps.tech Plugins Information") ?>
</h3>
<br />
...
<div class="tabs" style="height:10px;"></div>
		...
		<ul class='seaocore_browse'>
			...
			<?php 
				// No plugin installed of 'SocialApps.tech' in the site then show the msg.
				if( !empty($install_plugin) ) {
					if( empty($this->channel) ) {
				echo '<div class="tip"><span>No plugins by SocialApps.tech were found on your site. Click <a href="https://socialapps.tech/catalog/1/plugins" target="_blank">here</a> to view and purchase them.</span></div>';
					}
				}  
				?>
				<?php foreach( $this->channel as $item ): ?>
					...
					if( empty($item['running_version']) ) {
						...
						$running_version = 0;
							...
							$should_do = '<a href="https://socialapps.tech/user" class="seaocore_type_seaocore" target="_blank">' . $this->translate('Download Latest Version') . '</a>';
						} else {
        ...
        <li>
          ...
          <div class="seaocore_options">
					  ...
					  ?>
						<?php echo '<a href="http://demo.SocialApps.tech" class="seaocore_type_seaocore" target="_blank">' . $this->translate('Demo') . '</a>'; ?>
