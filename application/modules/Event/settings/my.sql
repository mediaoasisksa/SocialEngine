...
('core_main_event', 'event', 'Events', '', '{"route":"event_general","icon":"fa fa-calendar"}', 'core_main', '', 6),

('core_sitemap_event', 'event', 'Events', '', '{"route":"event_general"}', 'core_sitemap', '', 6),

('event_main_upcoming', 'event', 'Upcoming Events', '', '{"route":"event_upcoming","icon":"fa fa-calendar-plus"}', 'event_main', '', 1),
('event_main_past', 'event', 'Past Events', '', '{"route":"event_past","icon":"fa fa-calendar-minus"}', 'event_main', '', 2),
('event_main_manage', 'event', 'My Events', 'Event_Plugin_Menus', '{"route":"event_general","action":"manage","icon":"fa fa-user"}', 'event_main', '', 3),
('event_main_create', 'event', 'Create New Event', 'Event_Plugin_Menus', '{"route":"event_general","action":"create","icon":"fa fa-plus"}', 'event_main', '', 4),
...
-- ALL
-- auth_view, auth_comment, auth_photo, auth_html
INSERT IGNORE INTO `engine4_authorization_permissions`
  SELECT
    ...
    'auth_view' as `name`,
    5 as `value`,
    '["everyone","registered","owner_network","owner_member_member","owner_member","parent_member","member"]' as `params`
...
INSERT IGNORE INTO `engine4_authorization_permissions`
  SELECT
    ...
    5 as `value`,
    '["registered","owner_network","owner_member_member","owner_member","parent_member","member","owner"]' as `params`
  FROM `engine4_authorization_levels` WHERE `type` NOT IN('public');
INSERT IGNORE INTO `engine4_authorization_permissions`
  SELECT
    level_id as `level_id`,
