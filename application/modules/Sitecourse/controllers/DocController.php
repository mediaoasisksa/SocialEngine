...
class Sitecourse_DocController extends Core_Controller_Action_Standard {
	...
	public function addLessonVideoInviteAction() {
    	// In smoothbox
		$this->_helper->layout->setLayout('default-simple');
    	...
    	// Check post
		if( !$this->getRequest()->isPost() ) {
			$this->renderScript('doc/doclesson_form_video.tpl');
			return;
		}
		if( !$form->isValid($this->getRequest()->getPost()) ) {
			$this->renderScript('doc/doclesson_form_video.tpl');
		...
		try {
			$values = array_merge($value,array('topic_id'=>$topic_id,'order'=>9999,'type'=>'videoinvite','course_id'=>$course['course_id'],));
			$lesson = $table->createRow();
			$lesson->setFromArray($values);
			$lesson->save();
