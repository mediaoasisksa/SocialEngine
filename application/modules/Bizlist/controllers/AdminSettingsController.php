 ...
 * @license    http://www.socialengine.com/license/
 */
class Bizlist_AdminSettingsController extends Core_Controller_Action_Admin
{
  public function indexAction()
  {
    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
      ->getNavigation('bizlist_admin_main', array(), 'bizlist_admin_main_settings');

    $this->view->form = $form = new Bizlist_Form_Admin_Global();
  ...
  public function editCategoryAction()
  {
    ...
    $form = $this->view->form = new Bizlist_Form_Admin_Category();
    ...
    if( $this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost()) ) {
      ...
      try
      {
        ...
        if(isset($_POST['parentcategory_id']) && !empty($_POST['parentcategory_id'])) {
          $categoryItem = Engine_Api::_()->getItem('bizlist_category', $_POST['parentcategory_id']);
  ...
  public function categoriesAction() {
    ...
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('bizlist_admin_main', array(), 'bizlist_admin_main_categories');
    Engine_Api::_()->getApi('categories', 'core')->categories(array('module' => 'bizlist'));
  }
  ...
  public function changeOrderAction() {
    ...
    if ($this->_getParam('id', false) || $this->_getParam('nextid', false)) {
      ...
      $categoryTypeId = '';
      $checkTypeCategory = $dbObject->query("SELECT * FROM engine4_bizlist_categories WHERE category_id = " . $category_id)->fetchAll();
      ...
      if ($checkTypeCategory)
        $currentOrder = Engine_Api::_()->getDbtable('categories', 'bizlist')->order($categoryTypeId, $categoryType);
      ...
      $checkCategoryChildrenCondition = $dbObject->query("SELECT * FROM engine4_bizlist_categories WHERE subcat_id = '" . $id . "' || subsubcat_id = '" . $id . "' || subcat_id = '" . $nextid . "' || subsubcat_id = '" . $nextid . "'")->fetchAll();
