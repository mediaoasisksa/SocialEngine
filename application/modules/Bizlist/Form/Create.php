 ...
 * @license    http://www.socialengine.com/license/
 */
class Bizlist_Form_Create extends Engine_Form
{   
    ...
    public function init()
    {
        $this->setTitle('Post New Listing')
            ->setDescription('Compose your new business listing below, then click "Post Listing" to publish the listing.')
            ->setAttrib('name', 'bizlists_create');
        ...
        // Element: description
        $allowedHtml = Engine_Api::_()->authorization()->getPermission($userLevel, 'bizlist', 'auth_html');
        $uploadUrl = "";
        ...
        if( $allowedHtml && Engine_Api::_()->authorization()->isAllowed('album', $viewer, 'create') ) {
            $uploadUrl = Zend_Controller_Front::getInstance()->getRouter()
                ->assemble(array('controller' => 'index', 'action' => 'upload-photo'), 'bizlist_general', true);
        }
        ...
        $allowedUpload = Engine_Api::_()->authorization()->getPermission($userLevel, 'bizlist', 'photo');
        if( $allowedUpload ) {
        ...
        // Add subforms
        if( !$this->_item ) {
            $customFields = new Bizlist_Form_Custom_Fields([
        ...
        if( get_class($this) == 'Bizlist_Form_Create' ) {
        ...
        if (Engine_Api::_()->authorization()->isAllowed('bizlist', $user, 'allow_network')) {
        ...
        // View
        $viewOptions = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('bizlist', $user, 'auth_view');
        ...
        if( !empty($viewOptions) && engine_count($viewOptions) >= 1 ) {
            ...
            if( engine_count($viewOptions) == 1 ) {
                ...
                $this->addElement('Select', 'auth_view', array(
                    'label' => 'Privacy',
                    'description' => 'Who may see this business listing?',
        ...
        // Comment
        $commentOptions = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('bizlist', $user, 'auth_comment');
        ...
        if( !empty($commentOptions) && engine_count($commentOptions) >= 1 ) {
            ...
            if( engine_count($commentOptions) == 1 ) {
                ...
                $this->addElement('Select', 'auth_comment', array(
                    ...
                    'description' => 'Who may post comments on this business listing?',
...
//         }
        ...
        $this->addElement('Cancel', 'cancel', array(
            ...
            'link' => true,
            'prependText' => ' or ',
            'href' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('action' => 'manage'), 'bizlist_general', true),
            'onclick' => '',
            'decorators' => array(
                'ViewHelper',
            ),
