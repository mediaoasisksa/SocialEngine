...
<div class="layout_middle">
  <div class="generic_layout_container">
      <div class="headline">
        <h2>
          <?php echo $this->translate('Business Listings');?>
        </h2>
        <div class="tabs">
          <?php
            // Render the menu
    ...
    <div class="bizlist_form">
      <form action="<?php echo $this->escape($this->form->getAction()) ?>" method="<?php echo $this->escape($this->form->getMethod()) ?>" class="global_form">
        <div>
          ...
          <?php echo $this->form->bizlist_id; ?>
          <ul class='bizlists_editphotos'>
            <?php foreach( $this->paginator as $photo ): ?>
            <li>
              <div class="bizlists_editphotos_photo">
                <?php echo $this->itemPhoto($photo, 'thumb.normal')  ?>
              </div>
              <div class="bizlists_editphotos_info">
                <?php
                    $key = $photo->getGuid();
                echo $this->form->getSubForm($key)->render($this);
                ?>
                <div class="bizlists_editphotos_cover">
                  <input type="radio" name="cover" value="<?php echo $photo->getIdentity() ?>" <?php if( $this->bizlist->photo_id == $photo->file_id ): ?> checked="checked"<?php endif; ?> />
                </div>
                <div class="bizlists_editphotos_label">
...
<script type="application/javascript">
  scriptJquery(document).ready(function() {
    scriptJquery('.bizlist_photos').each(function(){
