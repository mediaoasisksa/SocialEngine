 ...
 * @license    http://www.socialengine.com/license/
 */
class Bizlist_Model_DbTable_Categories extends Engine_Db_Table
{
  protected $_rowClass = 'Bizlist_Model_Category';
  ...
  public function getUserCategoriesAssoc($user)
  {
    ...
    $stmt = $this->getAdapter()
        ->select()
        ->from('engine4_bizlist_categories', array('category_id', 'category_name'))
        ->joinLeft('engine4_bizlist_bizlists', "engine4_bizlist_bizlists.category_id = engine4_bizlist_categories.category_id")
        ->group("engine4_bizlist_categories.category_id")
        ->where('engine4_bizlist_bizlists.owner_id = ?', $user)
        ->where('engine4_bizlist_bizlists.draft = ?', "0")
        ->order('category_name ASC')
        ->query();
