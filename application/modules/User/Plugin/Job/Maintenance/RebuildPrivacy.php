 ...
 * @license    http://www.socialengine.com/license/
 */
class User_Plugin_Job_Maintenance_RebuildPrivacy extends Core_Plugin_Job_Abstract
{
  protected function _execute()
  {
    ...
    while( !$break && $count <= $limit ) {
      ...
      // Main
        ...
        if( !$maxViewRole ) $maxViewRole = ( engine_count($view_options) > 0 ? $view_options[count($view_options) - 1] : 'everyone' );
        if( !$maxCommentRole ) $maxCommentRole = ( engine_count($comment_options) > 0 ? $comment_options[count($comment_options) - 1] : 'everyone' );
