...
en4.seaocore.popupLoginSignup = {
  ...
  setLoginForm: function (el) {
    ...
    var handelerOnFocus = function (event) {
      scriptJquery(event.target).parent('.form-wrapper').addClass('form-wapper-focus');
    };
    var handelerOnBlur = function (event) {
      scriptJquery(event.target).parent('.form-wrapper').removeClass('form-wapper-focus');
    };
    if (el.find("#twitter-wrapper")[0] || el.find("#facebook-wrapper")[0]) {
      var wrapperDiv = document.createElement("div");
      wrapperDiv.id = "seaocore_loginform_sociallinks";
      wrapperDiv.appendTo(el);
      if (el.find("#facebook-wrapper")[0]) {
        el.find("#facebook-element").title = en4.core.language.translate("Login with Facebook");
        el.find("#facebook-wrapper").appendTo(wrapperDiv);
      }

      if (el.find("#twitter-wrapper")[0]) {
        el.find("#twitter-element").title = en4.core.language.translate("Login with Twitter");
        el.find("#twitter-wrapper").appendTo(wrapperDiv);
      }
    }

    el.find('#input').each(function (el) {
      var type = el.get('type');
      if (type == 'email') {
        el.parent('.form-wrapper').addClass('form-email-wrapper');
      }
      if (el.attr('id') == 'password') {
        ...
        }).appendTo(el.parent('.form-element'));
      ...
      if ((type == 'text' || type == 'email' || type == 'password') && el.parents('.form-wrapper').find('label').html()) {
        el.attr('placeholder', el.parents('.form-wrapper').find('label').html());
        el.parents('.form-wrapper').addClass('_slpff');
        el.on('focus', handelerOnFocus);
        el.on('blur', handelerOnBlur);
      }
    });
  },
  ...
  setSignupForm: function (formEl) {
    ...
    var handelerOnFocus = function (event) {
      scriptJquery('#'+event.target).parents('.form-wrapper').addClass('form-wapper-focus');
    };
    var handelerOnBlur = function (event) {
      scriptJquery('#'+event.target).parents('.form-wrapper').removeClass('form-wapper-focus');
    };
    if (formEl.find("#twitter-wrapper")[0] || formEl.find("#facebook-wrapper")[0]) {
      var wrapperDiv = document.createElement("div");
      wrapperDiv.id = "seaocore_signupform_sociallinks";
      wrapperDiv.appendTo(formEl, 'top');
      if (formEl.find("#facebook-wrapper")[0]) {
      ...
      if (formEl.find("#twitter-wrapper")[0]) {
        var wrapperDiv = document.createElement("span");
    ...
    if (wrapperDiv && wrapperDiv.find('.plan_subscriptions_container')) {
      className = className + ' seaocore_seaolightbox_plan_subscriptions';
    }

    formEl.find('.input').each(function (el) {
      if(el.hasAttribute('icon') &&  (el.getAttribute('icon') == 'fa fa-user' || el.getAttribute('icon') == 'fa-user') ){
        el.parents('.form-wrapper').addClass('form-user-wrapper');
      }
      var type = el.get('type');
      if (type == 'email') {
        el.parents('.form-wrapper').addClass('form-email-wrapper');
      }
      if ((type == 'text' || type == 'email' || type == 'password') && el.parents('.form-wrapper').find('label').html()) {
        el.attr('placeholder', el.parents('.form-wrapper').find('label').html());
        el.parents('.form-wrapper').addClass('_sspff');
        el.on('focus', handelerOnFocus);
        el.on('blur', handelerOnBlur);
      }
    });
    if (formEl.find('#password-element') && formEl.find('#passconf-element')) {
      formEl.find('#password-element').parents('.form-wrapper').addClass('_spfhf');
      formEl.find('#passconf-element').parents('.form-wrapper').addClass('_spfhf');
    }
    ...
    if (timezoneEl && !formEl.find('#timezone-option-label')) {
      ...
      var options = scriptJquery.crtEle('option', {
        ...
        'html': el.parents('.form-wrapper').find('label').html()

      });
      options.appendTo(el, 'top');
      el.parents('.form-wrapper').addClass('_sspff');
      if (canMakeSmallFileds) {
        el.parents('.form-wrapper').addClass('_spfhf');
      }
    }
    if (languageEl && !formEl.find('#language-option-label')) {
    ...
    if (formEl.find('#profile_type') && formEl.find('#profile_type').get('type') != 'hidden' && !formEl.find('#profile_type-option-label')) {
      var el = formEl.find('#profile_type');
      var addedFields = false;
      el.finds('option').each(function (el) {
        if (!el.attr('value').trim()) {
          el.html(el.parents('.form-wrapper').find('label').html().addClass('_sspff_option_label'));
          addedFields = true;
        }
      });
      if (!addedFields) {
      ...
      el.parents('.form-wrapper').addClass('_sspff seaocore_popup_profile_type_form_field');
    }
  },
      ...
      formElement.addEventListener((en4.seaocore.browser.msieversion()) ? 'keydown' : 'keypress', function (event) {
        if (event.shift && event.key == 'enter') {
          ...
          if (scriptJquery("#comment-form-loading-li_" + type + '_' + id).length)
            scriptJquery("#comment-form-loading-li_" + type + '_' + id).css( 'display', "block" );
...
en4.seaocore.miniMenu = {
  ...
  pullDownOpenHandler: function (event) {
    ...
    var newContentRequest = scriptJquery.ajax({
      ...
      success: function (responseText, responseTree, responseElements, responseHTML, responseJavaScript)
      {
