...
SEAOLink = function(){
  ...
  this.initialize = function(options) {
	...
	this.setLinkName =  function(value) {
		if(value == 1) {
			this.activate();
		} else {
  ...
  this.activate = function() {
    ...
    // Generate form
		...
		if(!document.getElementById('seao_link_form_wrapper')) {			
			var temp_div = scriptJquery.crtEle('div', {
					'id' : 'seao_link_form_wrapper',
					'class' : 'form-wrapper seao_link_form_wrapper'
			}).appendTo(formElements, 'before');
			
			var temp_div_label = scriptJquery.crtEle('div', {
				'class' : 'form-label'
			}).appendTo(temp_div);
			
			var label = scriptJquery.crtEle('label', {
				'for' : 'seao_link_label',
				'id' : 'seao_link_label',
				'html' : this._lang('Enter URL of Website'),
			}).appendTo(temp_div_label);
			
			var temp_div_element = scriptJquery.crtEle('div', {
				'class' : 'form-element'
			}).appendTo(temp_div);
			...
			var input =  scriptJquery.crtEle('input', {
				...
				'autocomplete' : 'off',
				'type' : 'text'
			}).appendTo(temp_div_element);

			formElements.formSubmit = scriptJquery.crtEle('button', {
		...
		}).appendTo(this.elements.body, 'after');
  },
	...
	this.makeLoading = function(action) {
        ...
        element.css('height', '0px').css('visibility', 'hidden');
			...
			}).appendTo(this.elements.loading);
		}
	},
  ...
  this.doAttach = function() {
    ...
    // Send request
    this.request = scriptJquery.ajax(options);
    this.request.send();
  },
...
//       }).appendTo(this.elements.previewNoImage);
			...
			this.elements.NoImageInput = scriptJquery.crtEle('input', {
				'type' : 'hidden',
				'events' : {
					'click' : this.prefield()
				}
  ...
  this.setImageThumb = function(element) {
    ...
    if( element ) {
      ...
      if( this.elements.previewCount ) {
        ...
        //this.elements.previewCount.seaoset('html', ' | ' + (index + 1) + ' of ' + this.params.loadedImages.length + ' | ');
	if ( index < 0 ) { index = 0; }
        this.elements.previewCount.seaoset('html', ' | ' + this._lang('%d of %d', index + 1, this.params.loadedImages.length) + ' | ');
      }
    } else {
