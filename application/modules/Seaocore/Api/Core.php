...
class Seaocore_Api_Core extends Core_Api_Abstract {
    ...
    public function isCdn() {
        ...
        $cdn_path = null;
        $checkVersion = $this->checkVersion($storageversion, '4.1.6');
...
//     }
    ...
    public function isLessThan420ActivityModule() {
        ...
        $checkVersion = $this->checkVersion($activityModuleVersion, '4.1.8');
        if (empty($checkVersion)) {
            return true;
        } else {
            return false;
    ...
    public function isSitePWAMode() {
      return Engine_Api::_()->hasModuleBootstrap('sitepwa') && Engine_API::_()->sitepwa()->hasEnabledPWAmode();
    }
    ...
    public function tinymceEditorOptions($upload_url = false) {
        ...
        $checkVersion = $this->checkVersion($coreversion, '4.7.0');
        if (empty($checkVersion)) {
            return array(
            ...
            $checkVersion = $this->checkVersion($coreversion, '4.9.0');
            if (!empty($checkVersion)) {
    ...
    public function getLocationsTabs() {
        ...
        $modules = array( 'siteevent', 'sitealbum', 'sitemember', 'sitepage', 'sitebusiness', 'sitegroup', 'sitestore', 'sitequicksignup');
  ...
  //FUNCTION FOR CLEARING THE MENUS CACHE
    ...
    public function getValueOfCssVariables($file, $search) {
        ...
        if (!empty($file) && !empty($search)) {
            ...
            if (!empty($tempValue)){
                $tempValues = explode(":", $tempValue);
                $explodeVariableValue = (int) str_replace("px;", "", end($tempValues));
            }
        }
    ...
    public function createCoreVideo($params, $file, $values) {
