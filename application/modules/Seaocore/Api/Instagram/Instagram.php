...
class Instagram {
     ...
     * @var array
     */
    private $_scopes = array('basic', 'public_content');
    ...
    /**
     * User Self Feeds (END POINT IS NOW DEPRECATED)
     *
     * @param boolean $signedHeader
     * @return void
     */
    public function getUserFeedNext($limit = 0, $maxId = null, $minId = null) {
        return $this->_makeCall('users/self/feed', true, array('count' => $limit, 'min_id' => $minId, 'max_id' => $maxId));
    }

    /**
     * RETURNS THE NEXT SET OF USER LIKED FEEDS
     *
     * @param varchar $function: url of the request
     * @return json decoded data of feeds
     */
    public function getPaginatorNext($function) {

        // signed header of POST/DELETE requests
        $headerData = array('Accept: application/json');

        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $function);
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headerData);
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 20);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
