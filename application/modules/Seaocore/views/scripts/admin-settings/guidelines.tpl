...
ACTION<br />$this->view->editAction = "edit";<br /><br />//GET COMMENT<br />$this->view->canComment = 1;<br />break</b>;');?><br />
	...
	<?php echo $this->translate("b) Go to the line around line number 37 having: <b style='font-weight:bold;'>if(scriptJquery('.feed_attachment_event_photo')) { </b>.");?><br />
	...
	<?php echo $this->translate("d) Paste this code at around line number 46, i.e., after the line mentioned in the previous point, after modifying the code according to the desired module / plugin. I.e., add the code for the desired module / plugin, similar to the code for Events: <br /><b style='font-weight:bold;'>if(scriptJquery('.feed_attachment_event_photo')) {<br />//DISPLAY ACTIVITY FEED IMAGES IN THE LIGHTBOX FOR THE EVENT <br />if(scriptJquery('.feed_attachment_event_photo')) {<br />scriptJquery('.feed_attachment_event_photo').each(function(el){<br />el.getElement('.thumb_normal').removeEvents('click').addEvent('click', function(e) {<br />e.stop();<br />href = openLightboxforActivityFeed(el);<br />openSeaocoreLightBox(href);<br />});<br />});<br />}</b>;")?>
