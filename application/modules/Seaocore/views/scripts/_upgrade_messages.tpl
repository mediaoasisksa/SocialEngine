...
function dismiss(modName) {
  ...
  document.cookie = modName + "_dismiss" + "=" + 1 + "; " + expires;
	scriptJquery('#dismiss_modules').css('display', 'none');
}

function confirmGoogle() {   
	 var r=confirm("Are you sure you have edited your Google Application?");
   if (r==true) {  
     scriptJquery('#gmail_redirecturl_confirm').css('display', 'none');
...
<div style="float:right;">
	<button onclick="dismiss('<?php echo $moduleName; ?>');"><?php echo $this->translate('Dismiss'); ?></button>
		...
		<div class="seaocore-notice-text ">
			<?php echo 'To see the latest available version for this plugin and more information, please visit the "Plugins" > "SocialApps.tech" section of your site\'s Admin Panel.'; ?>
		</div>	
	</div>
</div>
<?php endif; ?>
