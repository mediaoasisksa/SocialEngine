...
<?php //if(!empty ($this->showViewMore)): ?>
  <script type="text/javascript">
    ...
    function hideViewMoreLink(){
        if(scriptJquery('#mutual_friend_pops_view_more'))
            scriptJquery('#mutual_friend_pops_view_more').style.display = '<?php echo ( $this->paginator->count() == $this->paginator->getCurrentPageNumber() || $this->count == 0 ? 'none' : '' ) ?>';
    }
