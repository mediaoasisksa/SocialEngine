...
<div class="photo_lightbox_cont">
  ...
  <div class="lightbox_btm_bl">
    ...
    new Asset.javascript('http://assets.pinterest.com/js/pinit.js', {});
        ...
        en4.photoadvlightbox.flip(<?php echo $this->photo->getIdentity() ?>, 'horizontal', '<?php echo $this->resource_type; ?>').addEvent('complete', function() {
          this.attr('class', 'icon_photos_lightbox_flip_horizontal')
        }.bind(this));" title="<?php echo $this->translate("Flip Horizontal"); ?>" ></a>
            <a class="icon_photos_lightbox_flip_vertical" onclick="$(this).attr('class', 'icon_loading');
        en4.photoadvlightbox.flip(<?php echo $this->photo->getIdentity() ?>, 'vertical', '<?php echo $this->resource_type; ?>').addEvent('complete', function() {
          this.attr('class', 'icon_photos_lightbox_flip_vertical')
        ...
        scriptJquery('#comment-form_<?php echo $this->subject()->getType() . "_" . $this->subject()->getIdentity() ?>').style.display = '';
  ...
  function getTaggerInstanceSEAO() {
    if (!scriptJquery('#media_photo_next'))
      return;
    ...
    var onclickNext = scriptJquery('#media_photo_next').getProperty('onclick');
    taggerInstanceSEAO.addEvents({
      'onBegin': function() {
        scriptJquery('#media_photo_next').setProperty('onclick', '');
      },
      'onEnd': function() {
        scriptJquery('#media_photo_next').setProperty('onclick', onclickNext);
      },
  ...
  var keyDownEventsSEAOCorePhoto = function(e) {
    if (e.target.get('tag') == 'html' ||
            ...
            (e.target.get('tag') == 'div' && !e.target.parents('.compose-container')) ||
...
<?php if ($this->canComment && $this->viewPermission): ?>
      if (e.key == 'l' && (
        ...
        if (scriptJquery('#'+photo_like_id + "unlike_link") && scriptJquery('#'+photo_like_id + "unlike_link").css('display') == "none") {
          en4.seaocore.likes.like('<?php echo $this->subject()->getType() ?>', '<?php echo $this->subject()->getIdentity() ?>', '1');
        } else if (scriptJquery('#'+photo_like_id + "like_link") && scriptJquery('#'+photo_like_id + "like_link").css('display') == "none") {
...
<script type="text/javascript">
    ...
    function checkPasswordProtection(obj) {
        ...
        var flag = true;
        if (scriptJquery('#password_error'))
            scriptJquery('#password_error').destroy();

        if (obj['password'] && obj['password'].val() == '') {
            liElement = new Element('span', {'html': '<?php echo $this->translate("* Please complete this field - it is required."); ?>', 'class': 'sitealbum_protection_error', 'id': 'password_error'}).appendTo(scriptJquery('#password-element'));
            flag = false;
        }
            ...
            var request = scriptJquery.ajax({
                ...
                success: function (responseJSON) {
                    if (responseJSON.status == 0) {
                        ...
                        liElement = new Element('span', {'html': '<?php echo $this->translate("This is not valid password. Please try again."); ?>', 'class': 'sitealbum_protection_error', 'id': 'password_error'}).appendTo(scriptJquery('#password-element'));
                        flag = false;
                    } else {
                        closeSEAOLightBoxAlbum();
