...
<?php if ($this->canComment): ?>
  ...
  ?>onclick="en4.seaocore.photolightbox.comments.like('<?php echo $this->subject()->getType() ?>', '<?php echo $this->subject()->getIdentity() ?>');" href="javascript:void(0);" class="photo_lightbox_like"
    ...
    <a href="javascript:void(0);" onclick="if (scriptJquery('#comment-form-open-li_<?php echo $this->subject()->getType() ?>_<?php echo $this->subject()->getIdentity() ?>')) {
        scriptJquery('#comment-form-open-li_<?php echo $this->subject()->getType() ?>_<?php echo $this->subject()->getIdentity() ?>').style.display = 'none';
      }
      ...
      en4.core.request.send(scriptJquery.ajax({
        ...
        success: function(responseJSON) {
          if ($type(responseJSON) == 'object' && $type(responseJSON.status)) {
            if (scriptJquery('#'+type + 'like_link'))
              scriptJquery('#'+type + 'like_link').css('display', "none");
            if (scriptJquery('#'+type + 'unlike_link'))
