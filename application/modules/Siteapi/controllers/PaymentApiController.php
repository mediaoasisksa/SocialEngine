 ...
 */
class Siteapi_PaymentApiController extends Core_Controller_Action_Standard {
    ...
    public function init() {
        ...
        if (!$this->_user || !$this->_user->getIdentity()) {
            ...
            if (!$this->_user || !$this->_user->getIdentity()) {
                ...
                $this->_forward('finish', 'payment-api', 'siteapi', array(
                    'state' => 'unauthorized'
                ));
                return;
            }
    ...
    public function gatewayAction() {
        ...
        foreach ($gateways as $gateway) {
            ...
            if ((isset($gateway->plugin)) && !empty($gateway->plugin) && $gateway->plugin == 'Payment_Plugin_Gateway_2Checkout' || $gateway->plugin == 'Payment_Plugin_Gateway_PayPal')
                $gatewayPlugins[] = array(
                    'gateway' => $gateway,
    ...
    protected function _finishPayment($state = 'active') {
        ...
        // No user?
        if (!$this->_user) {
            return $this->_helper->redirector->gotoRoute(array('action' => 'finish', 'state' => 'unauthorized'));
        }
        ...
        if ($viewer->getIdentity() && !$viewer->enabled) {
            ...
            return $this->_helper->redirector->gotoRoute(array('action' => 'finish', 'state' => 'verified'));
        }
        ...
        // Redirect
        if ($state == 'free') {
            return $this->_helper->redirector->gotoRoute(array('action' => 'finish', 'state' => 'free'));
        } else {
