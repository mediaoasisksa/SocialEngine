...
$tokenTable = Engine_Api::_()->getItemTable('token');
    ...
    if(isset($params['token'])) {
        $select->join($tokenTableName, "$serviceTableName.owner_id = $tokenTableName.user_id");
    ...
    if (!empty($params['type']) && $params['type'] != -1) {
      $select->where($serviceTableName . '.type = ?', $params['type']);
    }
  ...
  public function serviceListTabs($params = array()) {
    ...
    if (isset($params['parent_parent_type'])) {
      $select->where($serviceTableName . '.parent_parent_type = ?', $params['parent_parent_type']);
    }  
    if (isset($params['parent_parent_id'])) {
      $select->where($serviceTableName . '.parent_parent_id = ?', $params['parent_parent_id']);
    }  
