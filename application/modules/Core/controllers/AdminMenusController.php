 ...
 * @version    $Id: AdminMenusController.php 9747 2012-07-26 02:08:08Z john $
 * @author     John
 */
...
class Core_AdminMenusController extends Core_Controller_Action_Admin
{
  ...
  public function indexAction()
  {
    ...
    $subMenus = array();
    $subMenuDisabled = array();
    $menusTable = Engine_Api::_()->getDbtable('menus', 'core');
    if($name == "core_main"){
      foreach( $menuItems as $menu ) {
        if($menu->name == "core_main_invite" || $menu->name == "core_main_home" ||  $menu->name == "core_main_chat" || $menu->name == "core_main_invite"){
          $subMenuDisabled[$menu->name] = $menu->name;
          continue;
        }
        if($menu->custom){
          $menuName = $menu->name;
        }else{
          $menuName = $menu->module.'_main';
        }        
        $menuItemsSelect = $menusTable->select()
          ->where('name = ?', $menuName)
          ->limit(1);
          $count = engine_count($menusTable->fetchAll($menuItemsSelect));
        $subMenus[$menu->name]['count'] = $count ? $menuName : false;
        $subMenus[$menu->name]['name'] =  $menuName;
      }
    }
    $this->view->subMenuDisabled = $subMenuDisabled;
  ...
  {
    // Get form
    $this->view->form = $form = new Core_Form_Admin_Menu_SubmenuCreate();
    ...
    $menuItem->submenu = '';
    $menuItem->custom = 1;
    $menuItem->name = "temrorary";
    $menuItem->save();

    $menuItem->name = 'custom_' . sprintf('%d', $menuItem->id);
    $menuItem->save();

      $this->view->selectedMenu = $menu;
