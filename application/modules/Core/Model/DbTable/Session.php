 ...
 * @version    $Id: Session.php 7904 2010-12-03 03:36:14Z john $
 */

require_once 'Engine/Db/Table.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Core_Model_DbTable_Session extends Engine_Db_Table implements Zend_Session_SaveHandler_Interface
{
    ...
    const AUTH_COLUMN       = 'authIdentityColumn';
     ...
     * @var array
     */
    protected $_primaryAssignment = array('sessionId');
    ...
    /**
     * Session table auth id column
     *
     * @var string
     */
    protected $_authIdentityColumn = 'user_id';
    ...
    public function write($id, $data)
    {
        ...
        // Save auth identity in table
        if( !empty($_SESSION['Zend_Auth']['storage']) &&
            is_numeric($_SESSION['Zend_Auth']['storage']) ) {
          $data[$this->_authIdentityColumn] = $_SESSION['Zend_Auth']['storage'];
        }
    ...
    public function gc($maxlifetime)
    {
        $this->delete($this->getAdapter()->quoteIdentifier($this->_modifiedColumn) . ' + '
                    ...
                    . $this->getAdapter()->quote($maxlifetime));

        return true;
    }

    public function removeSessionByAuthId($authId)
    {
      $this->delete(array(
        'user_id = ?' => $authId,
      ));
