...
class Chat_AjaxController extends Core_Controller_Action_Standard
{
  public function pingAction()
  {
    ...
    if($fresh == 'false' && engine_count($events) > 0)
      $this->view->newmessage = 1;
  ...
  public function sendAction()
  {
    ...
    // Check count
    $rate = engine_count($session->rate);
    if( $rate > 10 ) {
      $this->view->status = false;
      $this->view->error = true;
  ...
  public function whisperAction()
  {
    ...
    // Check count
    $rate = engine_count($session->whisperRate);
    if( $rate > 10 ) {
      $this->view->status = false;
      $this->view->error = true;
      $this->view->message = 'RATE_LIMIT_EXCEEDED';
  ...
  public function statusAction()
  {
    ...
    if( !engine_in_array($state, array(0, 1, 2)) ) {
      $this->view->status = false;
      $this->view->error = true;
