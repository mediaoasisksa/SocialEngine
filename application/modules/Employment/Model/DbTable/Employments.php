 ...
 * @version    $Id: Employments.php 9747 2012-07-26 02:08:08Z john $
 * @author     Jung
 */
 ...
 * @license    http://www.socialengine.com/license/
 */
class Employment_Model_DbTable_Employments extends Core_Model_Item_DbTable_Abstract
{
    protected $_rowClass = "Employment_Model_Employment";

    /**
     * Gets a paginator for employment listings
     *
     * @param Core_Model_Item_Abstract $user The user to get the messages for
     * @return Zend_Paginator
     */
    public function getEmploymentsPaginator($params = array(),
                                            $customParams = null)
    {
        $paginator = Zend_Paginator::factory($this->getEmploymentsSelect($params, $customParams));
    ...
    /**
     * Gets a select object for the user's employment entries
     *
     * @param Core_Model_Item_Abstract $user The user to get the messages for
     * @return Zend_Db_Table_Select
     */
    public function getEmploymentsSelect($params = array(), $customParams = null)
    {
        ...
        $searchTable = Engine_Api::_()->fields()->getTable('employment', 'search');
        ...
        } else if (!Engine_Api::_()->getApi('settings', 'core')->getSetting('employment.allow.unauthorized',0)){
            $param = array();
        ...
        if( isset($customParams) ) {
            $select = $select
                ->joinLeft($searchTableName, "$searchTableName.item_id = $tableName.employment_id", null);

            $searchParts = Engine_Api::_()->fields()->getSearchQuery('employment', $customParams);
        ...
        if( !empty($params['tag']) ) {
            $select
                ->joinLeft($tagMapsTableName, "$tagMapsTableName.resource_id = $tableName.employment_id", null)
                ->where($tagMapsTableName . '.resource_type = ?', 'employment')
    ...
    public function isEmploymentExists($category_id, $categoryType = 'category_id') {
      return $this->select()
              ->from($this->info('name'), 'employment_id')
